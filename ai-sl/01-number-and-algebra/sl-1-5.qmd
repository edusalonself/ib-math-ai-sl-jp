...
### 前半：laws of exponents（指数法則） {#exp-laws}
...
::: {#exm-exp-laws}
[Write each of the following as a single power.]{.q-en}
...
::: {#exm-exp-negative}
[Write each of the following without negative exponents.]{.q-en}
...
::: {#exm-log-convert}
**(a)** [Write $10^{3} = 1000$ in logarithmic form.]{.q-en}
...
**(b)** 電卓の `log` キーです（[GDCの使い方](#logkeys)）。
...
::: {#exm-log-solve}
[The population of a town is currently $5000$. It is increasing by $8\%$ each year.]{.q-en}
...
電卓の log テンプレート（[GDCの使い方](#logtemplate)）で底に $1.08$ を入れると、
...
::: {#exm-log-decibel}
[The loudness $L$, in decibels, of a sound of intensity $I$ watts per square metre is given by]{.q-en}
...
### 底を自分で決める（log テンプレート） {#logtemplate}
...
[1]{.ex-no} [Write each of the following as a single power.]{.q-en}
...
[2]{.ex-no} [Write each of the following without negative exponents.]{.q-en}
...
[3]{.ex-no} [Find the value of each of the following.]{.q-en}

**(a)** [$2^{-4}$]{.q-en}

**(b)** [$(-3)^{0}$]{.q-en}

**(c)** [$\left(\dfrac{1}{2}\right)^{-3}$]{.q-en}
...
[4]{.ex-no} [Write each of the following in the other form (exponential $\leftrightarrow$ logarithmic).]{.q-en}

**(a)** [$2^{6} = 64$]{.q-en}

**(b)** [$\log_{10} 0.001 = -3$]{.q-en}

**(c)** [$e^{5} = 148.4\ldots$]{.q-en}
...
[5]{.ex-no} [Find the value of each of the following, correct to three significant figures.]{.q-en}
...
[6]{.ex-no} [Solve the equation $3^{x} = 200$, giving your answer correct to three significant figures.]{.q-en}
...
$\texttt{nSolve}\left(3^{x} = 200,\ x\right)$ でも同じ答えが出ます。
:::
...
[7]{.ex-no} [The population of a city is currently $8000$ and is increasing by $5\%$ each year. Find the number of complete years after which the population first reaches $20\,000$.]{.q-en}
...
[8]{.ex-no} [A car is bought for $\$30\,000$. Its value depreciates by $15\%$ each year. Find the number of complete years after which its value first falls below $\$10\,000$.]{.q-en}
...
[9]{.ex-no} [The energy $E$, in joules, released by an earthquake of magnitude $M$ on the Richter scale is given by]{.q-en}
...
[10]{.ex-no} [$\$5000$ is invested at a nominal annual rate of $4.5\%$, compounded yearly.]{.q-en}

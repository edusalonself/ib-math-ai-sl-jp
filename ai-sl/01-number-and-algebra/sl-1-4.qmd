...
$$
\texttt{nSolve}\left(7000 = 5000 \times \left(1+\frac{r}{100}\right)^{6},\ r\right) \ \to \ 5.768\ldots
$$
...
公式集の式には**分母に $100$ があります。** パーセントを小数に直すのは、式のほうがやってくれます。**自分でもう一度割ってはいけません。**
...
$$
8000 \times \left(1 + \frac{3.5}{400}\right)^{20} \ \to \ 9522.718394\ldots
$$

$$
\texttt{ans} - 8000 \times \left(1 + \frac{3.5}{100}\right)^{5} \ \to \ 21.23\ldots
$$

分数を入れたら **`►` で分数の外へ**、指数を入れたら **`►` で指数の外へ**出てから続きを打ちます（入力の基本は [SL 1.1](sl-1-1.qmd#gdc-template)）。
...
$\texttt{nSolve}\left(15000 = 12000 \times \left(1+\frac{r}{100}\right)^{5},\ r\right)$ でも同じ答えが出ます。

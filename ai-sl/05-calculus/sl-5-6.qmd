...
::: {.callout-note}
## second derivative（$2$ 階導関数）は、AI SL では使いません
AA（Analysis and Approaches）では $f''(x)$ の符号で見分けます。**AI SL のシラバスには $f''$ が入っていません。**（AI **HL** では AHL 5.10 で扱います。）
...
::: {.callout-note}
## $f'(x)$ の**式**は出てきません
電卓は、$3x^{2}-12x+9$ のような**式**を返しません（[SL 5.3](sl-5-3.qmd#nderiv)）。
...
自分で作った $f'(x)$ の式があるなら、Calculator ページで解けます。`nSolve` は `menu → Algebra → Numerical Solve` から入れられます（[SL 1.8](../01-number-and-algebra/sl-1-8.qmd#nsolve)）。

- $\texttt{nSolve}\left(3x^{2} - 12x + 9 = 0,\ x,\ 0,\ 2\right)$
- $\texttt{nSolve}\left(3x^{2} - 12x + 9 = 0,\ x,\ 2,\ 5\right)$

`nSolve` は**解を $1$ つずつ**返すので、$2$ つあるときは範囲を変えて $2$ 回使います。

$2$ 次式なら、まとめて出す手もあります。`polyRoots` は `menu → Algebra → Polynomial Tools → Find Roots of Polynomial` から入れられます。

$\texttt{polyRoots}\left(3x^{2} - 12x + 9,\ x\right)$

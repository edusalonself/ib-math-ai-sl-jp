...
TI-Nspire CX II は、$f'(x)$ の**式**を返しません。返すのは、**ある 1 点での値**だけです。
...
$$
\left. \frac{d}{dx}\left(3x^{4} - 5x^{2} + 7x - 2\right) \right|_{x=2}
$$

**$83$** と返ります。@exm-sl53-terms の答えと一致しました ✓
...
自分で作った $f'(x)$ の式を、`nSolve` に入れます（`nSolve` は `menu → Algebra → Numerical Solve` から入れられます：[SL 1.8](../01-number-and-algebra/sl-1-8.qmd#nsolve)）。

- $\texttt{nSolve}\left(3x^{2} - 12x = 0,\ x,\ -1,\ 1\right)$
- $\texttt{nSolve}\left(3x^{2} - 12x = 0,\ x,\ 2,\ 6\right)$

`nSolve` は**解を 1 つずつ**返すので、解が 2 つあるときは範囲を変えて 2 回使います（[SL 5.2](sl-5-2.qmd#nsolve) と同じです）。

$f'(x)$ が 2 次式なら、`polyRoots` を使う手もあります。`polyRoots` は `menu → Algebra → Polynomial Tools → Find Roots of Polynomial` から入れられます。

$\texttt{polyRoots}\left(3x^{2} - 12x,\ x\right)$

...
critical value は、**「この $r$ で linear correlation があると言ってよいか」を判断するための値**です。strong・weak の境目ではありません。**必要なときは問題文に書いてあります。** 表を覚える必要はありません。
:::
...
| | **weak**（弱い） | 点がばらついている |
| | **little or no linear correlation** | 直線の並びが見えない |
...
| $r$ が $0$ に近い | little or no linear correlation |
...
目分量ではなく、**電卓が計算してくれる直線**が **regression line**（回帰直線）です。regression line は、[Topic 2 で学んだ linear function](../02-functions/sl-2-1.qmd)（$1$ 次関数）を、**実際のデータに当てはめた model** です。
...
## クリックすると開きます
ここでは、**なぜ回帰直線が平均点 $(\bar{x}, \bar{y})$ を通るのか**だけを説明します。

回帰直線は **least-squares regression line** といって、それぞれの点と直線との**縦のずれ**（**residual**、残差）をもとに引かれています。

このとき、**residual の和が $0$ になる**ように直線が決まります。上に外れた分と下に外れた分が、ちょうど釣り合っているということです。

上下のずれが釣り合う高さが $\bar{y}$ なので、直線は $(\bar{x}, \bar{y})$ を通ります。

**AI SL では、この証明は要りません。** 「回帰直線は平均点を通る」という性質を、検算に使えれば十分です。
::::
...
**(c)** $0.15$ は $0$ に近い。→ **little or no linear correlation**（または **very weak positive linear correlation**）

**(d)** $-0.41$ は $0$ と $-1$ の中間くらい。→ **weak negative**

**strong・moderate・weak の境目に、IB 共通の決まった数値はありません。** $r$ の値だけでなく、**散布図の見え方・問題の文脈・問題文の指示**を合わせて判断してください。

**critical value は、強さの境目ではありません。** 「この $r$ で linear correlation があると言ってよいか」という**有意性**を判断するための値で、必要なときは問題文に与えられます。
...
**(c)** *Little or no linear correlation (or very weak positive linear correlation).*
...
**(b)** 電卓の Linear Regression に入れると、$r$ も一緒に出ます（[GDCの使い方](#gdc-regression)）。
...
**$\bar{y}$ と一致しました。** 回帰直線は平均点 $(\bar{x}, \bar{y})$ を通るので、**これが一番早い検算**です。
...
**点の並びと直線が合っているか、目で確かめてください。** 直線が大きくずれていたら、$x$ と $y$ を逆に入れています。
...
**(b)** *Little or no linear correlation.*
...
**(b)** $0.07$ はほぼ $0$。**little or no linear correlation** です。
...
**(c)** [Describe the correlation.]{.q-en}

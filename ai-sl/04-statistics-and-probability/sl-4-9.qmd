...
**(a)** 左のはしから $56$ まで。**下限には $-\infty$** を入れます（[GDCの使い方](#infinity)）。

$\texttt{normCdf}\left(-\infty,\ 56,\ 50,\ 6\right)$
...
$\texttt{normCdf}\left(44,\ \infty,\ 50,\ 6\right)$
...
$\texttt{normCdf}\left(47,\ 56,\ 50,\ 6\right)$
...
$\texttt{normCdf}\left(-\infty,\ 105,\ 120,\ 15\right)$
...
$\texttt{normCdf}\left(110,\ 130,\ 120,\ 15\right)$
...
$\texttt{normCdf}\left(140,\ \infty,\ 120,\ 15\right)$
...
$\texttt{invNorm}\left(0.90,\ 42,\ 5\right)$
...
$\texttt{invNorm}\left(0.25,\ 42,\ 5\right)$
...
$\texttt{normCdf}\left(50,\ 70,\ 62,\ 11\right)$
...
$\texttt{invNorm}\left(0.85,\ 62,\ 11\right)$
...
::: {.callout-warning}
## 端のない側を、空欄のままにする
`normCdf` の欄は**4つ**です。片側だけの問題でも、**空いているほうに $\pm\infty$ を入れます**（$\pm 1E99$ でも同じです。[GDC](#infinity)）。
...
**この項目は、電卓の操作そのものが中身です。** シラバスは、確率も値も `technology` で求めるよう指定しています。**つまり GDC を使って求めます。**
...
::: {.callout-note appearance="simple" #infinity}
## $\infty$ の入れ方と、$1E99$ という書き方
$\infty$ は、キーボードのいちばん下、左端の **`π` のキー**を押すと開く**記号パレット**から選びます（[AHL 1.12a](../../ai-hl/01-number-and-algebra/ahl-1-12a.qmd#gdc-i) と同じパレットです）。$-\infty$ は、負号 `(-)` を打ってから $\infty$ を選びます。

**$1E99$ と入れても同じです。** $1E99$ は $1 \times 10^{99}$ という意味で、**「事実上の無限大」**として使う書き方です。$E$ は電卓の `EE` キー（$10$ の指数を入れるキー）で出します。古い教材や TI-84 の解説では、こちらの書き方をよく見かけます。
...
::: {.callout-tip collapse="true"}
## 解説
`more than` なので、**$45$ から右**です。上限に $\infty$ を入れます。

$\texttt{normCdf}\left(45,\ \infty,\ 40,\ 3\right)$
...
$\texttt{invNorm}\left(0.95,\ 40,\ 3\right)$
...
$\texttt{normCdf}\left(-\infty,\ 245,\ 250,\ 4\right)$
...
$\texttt{invNorm}\left(0.80,\ 55,\ 12\right)$
...
$\texttt{invNorm}\left(0.10,\ 68,\ 9\right)$

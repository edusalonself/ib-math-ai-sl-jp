...
#### median は「何番目か」を先に出します {#median-position}

「まん中の値」と言われても、$50$ 個あったらどれがまん中か分かりません。**先に「何番目を見ればよいか」を計算します。**

$$
(\text{median の位置}) = \frac{n + 1}{2} \ \text{番目}
$$ {#eq-sl43-medpos}

$n$ はデータの**個数**です。**必ず、小さい順に並べてから**数えてください。
...
$n = 8$ なら $\dfrac{8+1}{2} = 4.5$ 番目。**$4$ 番目と $5$ 番目のあいだ**なので、
...
standard deviation は**散らばりの大きさ**なので、負になりません。ですから $\lvert k \rvert$ 倍です。**$k > 0$ なら、そのまま $k$ 倍**と思って構いません（試験に出るのは、ほとんどこの場合です）。
...
::: {.callout-warning}
## variance だけ $k^2$ 倍です
$k$ 倍したとき、standard deviation は $\lvert k \rvert$ 倍ですが、**variance は $k^2$ 倍**になります。
...
**(a)** 電卓の One-Variable Statistics に入れます（[GDCの使い方](#rawdata)）。
...
**$2.14$ と書くと、聞かれたものと違う値**になります。$s_x$ のほうを写してしまった、ということになります。
...
standard deviation が $\lvert k \rvert$ 倍で、variance はその $2$ 乗だからです。
:::
...
```
doc → Settings → Document Settings → Display Digits → Float 6
```
...
**(b)** [Find the standard deviation, correct to three significant figures.]{.q-en}

**(c)** [Find the variance, correct to three significant figures.]{.q-en}
...
$6 - 5 = 1$ としないでください。データ全部が同じだけ左にずれるので、**点どうしの間隔は変わっていません**（@fig-sl43-changes）。
...
**検算**：もとの variance は $2.5^2 = 6.25$。$6.25 \times 9 = 56.25$ で、たしかに $3^2 = 9$ 倍になっています。
...
**この判断が問われる設問は多くありませんが、$\sigma_x$ と $s_x$ を取り違えた答えは毎回のように失点します。** 電卓の画面では、**小さいほうが $\sigma_x$** です。並び順ではなく、値の大小で見分けてください。
:::

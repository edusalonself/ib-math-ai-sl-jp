...
modelling process 全体は、ここでは**4段階**で整理します。Subject Guide の SL 2.6 Content 欄では、このうち**モデルを作り始めた後の部分**が、次の $3$ つの見出しで示されています。
...
: modelling process の4段階 {#tbl-sl26-stages}

**✓ の $3$ つが、Subject Guide の SL 2.6 に直接示されている見出し**です。$1$ つめの `Pose the real-world problem` は、その前段として本書で足しています。
...
::: {.callout-warning}
## 「$x$ が増えると $y$ が減る」だけでは決められません
単に $x$ が増えると $y$ が減るというだけでは、**inverse variation とは判断できません。**

exponential decay も、負の傾きをもつ linear model も、quadratic model の一部分も、どれも右下がりになります。**$xy$ や $yx^{2}$ が一定になるかどうか**まで確かめてください。
:::
...
- $f(x) = mx + c$ なら、$f(0) = c$
- $f(x) = ka^{x}$ なら、$f(0) = k$
- $f(x) = ka^{x} + c$ なら、$f(0) = k + c$（[SL 2.5](sl-2-5.qmd)）
...
$3$ つめは、[SL 2.5 で説明した「$x = 0$ のときの値は $k + c$」](sl-2-5.qmd#exp-asymptote)そのものです。$k$ だけを $500$ にしないでください。
...
$f(x) = ax^{2} + bx + c$ の $a$、$b$、$c$ を出すには、**異なる $x$ 座標をもつ点が $3$ つ**あればよいことになります。点を $1$ つ代入するごとに、独立した式が $1$ 本できるからです。
...
SL では、**生徒自身が non-linear regression を実行してモデルの式を求めることは要求されません。** ただし、問題で与えられた quadratic、exponential、sinusoidal などの non-linear model を**読み、使い、適切さを考えること**はあります。

後の [SL 4.4](../04-statistics-and-probability/sl-4-4.qmd) では、実測データに最もよく合う regression line を求める方法を学びます。**ここでは、与えられた点や条件を満たすようにモデルのパラメーターを求めます。**
:::
...
::: {.callout-tip}
## 「よく合っている」だけでは足りません
`Comment on` の答えには、**根拠を書いてください。** 根拠になるのは、@tbl-sl26-check の $3$ つです。

**数値が与えられている場合は、差や percentage error を根拠にできます。** 数値だけでなく、**データの shape** や**現実の context** も合わせて評価します。どれを使えるかは、問題文と与えられた情報によります。
...
## クリックすると開きます
ここでは、**なぜ点が $3$ つあれば quadratic model が決まるのか**だけを説明します。

$f(x) = ax^{2} + bx + c$ には、**分からないパラメーターが3つ**あります（$a$、$b$、$c$）。
...
**(b)** 変数3つの連立方程式なので、電卓で解きます（[GDCの使い方](#simeq)）。
...
::: {#exm-sl26-ferris}
[A Ferris wheel has a radius of 20 metres. The lowest seat on the wheel is 2 metres above the ground.]{.q-en}

[Sam gets on at the lowest seat, and the wheel starts to turn. The wheel takes 120 seconds to make one complete turn.]{.q-en}

[The height of Sam's seat above the ground, $h$ metres, at time $t$ seconds after the wheel starts, is modelled by]{.q-en}

$$
h(t) = a\cos(bt) + d
$$

**(a)** [Find the value of $d$ and the value of $a$.]{.q-en}

**(b)** [Show that $b = 3$.]{.q-en}

**(c)** [Find the height of Sam's seat 20 seconds after the wheel starts.]{.q-en}

**(d)** [The ride lasts 4 minutes. Write down a reasonable domain for the model, and give a reason for your answer.]{.q-en}
...
観覧車は **[sinusoidal model](sl-2-5.qmd#tbl-sl25-sin) の代表的な場面**です。同じ形がくり返しもどってくるからです。

![A Ferris wheel and its height model](img/sl-2-6-ferris.svg){#fig-sl26-ferris width=100%}

**(a)** まず **一番高いところと一番低いところ**を、絵から読み取ります。

- 一番低い（乗るところ）：$2$ m
- 一番高い（真上）：$2 + 20 \times 2 = 42$ m（半径 $20$ の $2$ 倍だけ上がります）

SL 2.5 の $\text{maximum} = d + |a|$、$\text{minimum} = d - |a|$ を、**$d$ と $|a|$ について解き直した形**を使います。

$$
d = \frac{\text{max} + \text{min}}{2} = \frac{42 + 2}{2} = 22,
\qquad
|a| = \frac{\text{max} - \text{min}}{2} = \frac{42 - 2}{2} = 20
$$

$d = 22$ は **[principal axis（中心線）](sl-2-5.qmd#tbl-sl25-sin)**、つまり**車輪の中心の高さ**です。$|a| = 20$ は **amplitude（振幅）**、つまり**半径**です。

あとは $a$ の**符号**です。ここが、この問題でいちばん差がつくところです。$t = 0$ で $\cos(0) = 1$ なので、[はじめの値](#route-initial)は

$$
h(0) = a \times 1 + 22 = a + 22
$$

Sam は**一番低いところ**から乗るので $h(0) = 2$。したがって
...
::: {.callout-important}
## $\cos$ で、下から始まるときは $a$ が負
$\cos$ のグラフは $t = 0$ で**一番上**にいます。ですから、

- $t = 0$ が**一番高い**ところ → $a$ は**正**
- $t = 0$ が**一番低い**ところ → $a$ は**負**（上下がひっくり返る）

観覧車は**乗るところが一番低い**ので、$a$ が負になります。$a = 20$ と答えてしまうのが、いちばん多い失点です。**$h(0)$ を計算して確かめてください。**
:::
...
**$b$ そのものは period ではありません**（[SL 2.5](sl-2-5.qmd#what-is-b)）。$\dfrac{360}{b}$ のほうが period です。
...
::: {.callout-warning}
## 電卓は **Degree** にしてください
$\dfrac{360}{b}$ の $360$ は度です。電卓が **Radian** のままだと $\cos(60)$ が $-0.952\ldots$ になり、答えが $41.0\ldots$ という**まったく違う数**になります。

SL では radian は要りません（SL 3.4）。**Degree のまま**にしておくのが安全です。
:::
...
これが [reasonable domain](#domain) です。
...
::: {.callout-note}
## 確かめておくとよいこと
$240 = 120 \times 2$ なので、**ちょうど $2$ 周**です。$h(240) = -20\cos(720^{\circ}) + 22 = -20 + 22 = 2$ となり、**乗ったところと同じ高さ**にもどります。式が正しく組めている証拠になります。
:::
...
::: {.callout-warning}
## 条件が足りないのに連立方程式を立てようとする
$ax^{2} + bx + c$ はパラメーターが $3$ つなので、**独立した条件が $3$ つ**要ります。異なる $x$ 座標をもつ点なら、$1$ 点で $1$ 本の式になります。
...
**(b)** $5$ 年で半分になるので $a^{5} = 0.5$。$a = 0.5^{\frac{1}{5}} = 0.8705\ldots$ です。$a < 1$ なので decay になっています。
...
**(b)** [The model predicts about 4.84 million users after 30 weeks. Comment on this prediction.]{.q-en}
...
[10]{.ex-no} [A student is given a set of data and asked to build a model. Describe, in order, the stages of the modelling process the student should follow.]{.q-en}

...
**この項目は、電卓の操作そのものが中身です。** 読むだけでは身につきません。`ctrl + doc → Add Graphs` を開いた状態で読み進めてください。
:::
...
```
menu → Analyze Graph → Zero          （x 切片・zeros・roots）
menu → Analyze Graph → Minimum       （最小点・下向きの vertex）
menu → Analyze Graph → Maximum       （最大点・上向きの vertex）
menu → Analyze Graph → Intersection  （2本のグラフの交点）
```
...
たとえば $f1(x) = a \times x^{2} + b \times x + c$ として $a$、$b$、$c$ をスライダーにすると、**$b$ を動かしたときに axis of symmetry がどう動くか**が目で見えます。

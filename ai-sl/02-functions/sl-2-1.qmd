...
シラバスの Content 欄にだけ書かれています。**ここは自分で覚えておく必要があります。** どちらも、**$2$ 本の gradient がどちらも定義されている場合**の式です（[第4節](#parallel-perp)）。
:::
...
::: {.callout-note}
## SL 4.4 では文字が違います
後の [SL 4.4](../04-statistics-and-probability/sl-4-4.qmd) では、regression line を $y = ax + b$ と書きます。**そちらでは $a$ が傾き**です。
...
$2$ 本の直線の gradient が**どちらも定義されている場合**、

$$
m_1 \times m_2 = -1
$$ {#eq-sl21-perp}

なら $2$ 直線は **perpendicular**（垂直）です。@fig-sl21-perp の右では $2 \times \left(-\dfrac{1}{2}\right) = -1$ になっています。

::: {.callout-warning}
## 水平と垂直の組だけは、この式で判定できません
水平な直線 $y = c$ と、垂直な直線 $x = k$ も、たがいに垂直です。ところが $x = k$ の gradient は **undefined**（[第1節](#gradient)）なので、掛け算そのものができません。
...
::: {.callout-tip}
## 「ひっくり返して、符号を変える」
$m_1 \neq 0$ で gradient が定義されているとき、垂直な直線の gradient は**その場で作れます。**
...
::: {.callout-note}
## この先、SL 3.5 で使います
ここで学んだ直線と perpendicular の知識は、[SL 3.5](../03-geometry-and-trigonometry/sl-3-5.qmd) で **perpendicular bisector**（垂直二等分線）を求めるときに使います。
:::
...
交点の左と右を指定すると、座標が出ます。`lower bound?` と `upper bound?` の $2$ 回、`enter` を押します（→ [SL 2.4](sl-2-4.qmd#bounds)）。

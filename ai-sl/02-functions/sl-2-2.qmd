...
|---|---|
| $f(x)$ | $x$ を入れたときの、$f$ の出力 |
| $v(t)$ | 時刻 $t$ での **velocity**（速度） |
| $C(n)$ | $n$ 個作ったときの**費用**（cost） |

: function notation の例 {#tbl-sl22-notation}

**velocity**（速度）は向きを含むので、**負になることがあります。** **speed**（速さ）は velocity の大きさです。
...
| 聞かれ方 | 試験での英語 | やること |
|---|---|---|
| **$f(4)$ を求めよ** | `Find` $f(4)$. | $x = 4$ を**入れて**、出力を出す |
| **$f(x) = 16$ を解け** | `Solve` $f(x) = 16$. | 出力が $16$ になる**入力を探す** |
...
**range は、式だけでなくグラフも併用すると判断しやすくなります。** 電卓でグラフをかいて、**「縦にどこからどこまで届いているか」**を見てください。

**domain は横の広がり、range は縦の広がり**です。

ただし、**GDC の画面に表示されるのはグラフの一部だけ**です（[SL 2.3](sl-2-3.qmd#screen-to-paper)）。domain、turning points、endpoints、asymptotes を確認して range を判断してください。
:::
...
ここからは、**$f$ が one-to-one で、inverse function をもつ場合**を考えます。どんな関数にも逆関数があるわけではありません。その条件は[この節のあと](#one-to-one)で見ます。
...
が成り立ちます。**入力と出力を入れかえるだけ**です。

$f(1) = 3$ なら、$f^{-1}(3) = 1$ です。**自分で書き換えられるようにしましょう。**

::: {.callout-important}
## $f$ が one-to-one で inverse function をもつ場合、$f(x) = 10$ を解くことと $f^{-1}(10)$ を求めることは同じです
シラバスに、この例がそのまま載っています。
...
#### グラフで見分けます ── horizontal line test {#horizontal-test}

![Checking whether a function is one-to-one](img/sl-2-2-horizontal-test.svg){#fig-sl22-hline width=100%}

@fig-sl22-hline がその見分け方です。**横の線を1本引いてみて、グラフと2回以上ぶつかったら one-to-one ではありません。** これを **horizontal line test**（水平線テスト）と呼びます。

- **左** … どこに横線を引いても、ぶつかるのは1回だけ → **one-to-one** → 逆関数がある
- **右** … $y = 4$ で2回ぶつかる → 出力1つに入力が2つ → **逆関数がない**

::: {.callout-tip}
## 縦の線と横の線で、調べていることが違います
名前が似ているので、**何を調べる線なのか**をはっきりさせておいてください。
...
: 2つのテスト {#tbl-sl22-tests}

縦の線は[はじめの節](#what-is-function)、横の線はここ。**順番も、この順で使います。** function でないものに、逆関数の話をしても意味がないからです。
:::

::: {.callout-note}
## domain をせまくすると、逆関数が持てます
$f(x) = x^{2}$ でも、**$x \geq 0$ に限れば**話が変わります。@fig-sl22-hline の右のグラフで、**$y$ 軸より右だけ**を見てください。どこに横線を引いても、ぶつかるのは1回だけです。

@tbl-sl22-swap で $x \geq 0$ という断りが付いていたのは、このためです。**domain を決めることが、逆関数を持てるかどうかを決めています。**
:::
...
## クリックすると開きます
ここでは、**なぜ $f^{-1}$ のグラフが $y = x$ の対称移動になるのか**だけを説明します。$f$ が one-to-one で、$f^{-1}$ が存在する場合の話です。
...
$f$ は one-to-one なので $f^{-1}$ があり、[$f(x) = 16$ を解くことと $f^{-1}(16)$ を求めることは同じ](#inverse)だからです。**$f^{-1}$ の式を作る必要はありません。**
...
**グラフをかいて確認するとよいです。** 電卓のグラフの入力欄に $f1(x) = \sqrt{2 - x}$ と入れれば（根号は `ctrl` + `x²` のテンプレート）、$x$ 軸の上にしか線がないことが目で見えます。
...
**(a)** 電卓に $f1(x) = x^{2} - 4x + 1$ と入れます（[GDCの使い方](#graph-range)）。**下に凸の谷の形（放物線）**が出ます。
...
カーソルが曲線に乗った状態で **$x$ の値を直接打ちこむ**と、そこへ飛んで座標が出ます。`0` と打てば $(0,\ 1)$、`5` と打てば $(5,\ 6)$ です。
...
`0` と打てば $(0,\ 3.5)$、`20` と打てば $(20,\ 39.5)$。**代入した値と合っていれば安心です。**
...
::: {.callout-warning}
## 座標を入れかえずに書く
$f^{-1}$ が存在するとき、$f$ が $(2,\ 5)$ を通るなら、$f^{-1}$ は **$(5,\ 2)$** を通ります。$(2,\ 5)$ のままではありません。
:::
...
左右の境目を指定すると、頂点の座標が出ます。`lower bound?` と `upper bound?` の $2$ 回、`enter` を押します（→ [SL 2.4](sl-2-4.qmd#bounds)）。
...
$f^{-1}$ が存在する場合、**$f^{-1}(k)$ を聞かれたら、方程式 $f(x) = k$ を解きます**（@eq-sl22-inv）。
...
SL 1.8 でやった **`nSolve`** でも解けます。ただし **`nSolve` が返すのは $1$ つだけ**です。
...
$\texttt{nSolve}\left(f1(x) = 6,\ x,\ 3,\ 8\right)$

`nSolve` は `menu → Algebra → Numerical Solve` から入れられます（[SL 1.8](../01-number-and-algebra/sl-1-8.qmd#nsolve)）。最後の $2$ つが「$x$ を $3$ から $8$ のあいだで探す」という指定です。
:::
...
**(b)** [逆関数があるのは one-to-one の関数だけ](#one-to-one)です。
...
**問2の $g(x) = x^{2} + 3$ と、ちょうど反対の例**です。読みくらべてみてください。
:::

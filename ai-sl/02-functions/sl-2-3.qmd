...
| 何を | どう書くか |
|---|---|
| **軸の名前**（axis labels） | $x$、$y$（文脈なら $t$、$h$ など**問題の文字**で） |
...
**(b)** $y = 0$ になる $x$ です。電卓のグラフで読みます（[GDCの使い方](#read-points)）。
...
**(b)** 電卓で `Analyze Graph → Maximum`（[GDCの使い方](#read-points)）。
...
**(d)** `f3(x) = 0.5x^2 - x + 3` を電卓に入れて（[GDCの使い方](#read-points)）、
...
境目は `lower bound?` と `upper bound?` の $2$ 回聞かれます。**`enter` を $2$ 回**押してください（→ [SL 2.4](sl-2-4.qmd#bounds)）。

...
Formula Booklet の SL 2.5 に載っているのは、quadratic function の axis of symmetry
...
linear、quadratic、exponential、direct / inverse variation、cubic、sinusoidal という**モデルの形は Subject Guide には示されていますが、Formula Booklet には載っていません。** 少なくとも、**式を見てモデルの種類と主な特徴を判断できるように**しておく必要があります。

sinusoidal model の次の $3$ つは、**Formula Booklet には載っていません。** 特に覚えてください。
...
| direct / inverse variation | $f(x) = ax^{n}$（$n$ は整数） | $x$ の累乗で決まる（落下距離・人数と時間） |
...
#### $k > 0$ の場合、growth か decay かは $a$ で判断できます {#growth-decay}
...
という条件が付きます。そのうえで、$f(x) = ka^{x} + c$ の動きは $k$ と $a$ の組み合わせで決まります。

| 条件 | グラフの動き |
|---|---|
| $k > 0$、$a > 1$ | increasing／**growth** |
| $k > 0$、$0 < a < 1$ | decreasing／**decay** |
| $k < 0$、$a > 1$ | **decreasing** |
| $k < 0$、$0 < a < 1$ | **increasing** |

: $ka^{x} + c$ の増加・減少 {#tbl-sl25-growthdecay}

**$k < 0$ ではグラフが上下に反転するため、増加・減少の向きも逆になります。** 実際の growth / decay model では $k > 0$ の場合が多いため、このページでは**まず $k > 0$ の例を中心に**扱います。

$ka^{-x} + c$ の形も、同じように読みます。$a^{-x} = \left(\dfrac{1}{a}\right)^{x}$ なので、$k > 0$ なら **$a > 1$ で decay、$0 < a < 1$ で growth** です。$x$ の前の符号で、向きが入れかわります。

$k\mathrm{e}^{rx} + c$ は、$k > 0$ なら $r > 0$ で growth、$r < 0$ で decay です。
...
$n$ が整数（$\mathbb{Z}$ は整数全体）であればよい、という形です。この形を **direct / inverse variation**（正比例・反比例）といいます。

$$
y = ax^{n}
$$

のうち、**$n = 1$ が日本の数学でふつう「比例」と呼ばれる $y = ax$**、**$n = -1$ がふつう「反比例」と呼ばれる $y = \dfrac{a}{x}$** です。

| $n$ | 呼び方 | 例 |
|---|---|---|
| $n > 0$ | **direct variation** | $y = 4.9x^{2}$（落下距離。**$y$ は $x^{2}$ に比例する**） |
| $n < 0$ | **inverse variation** | $y = \dfrac{120}{x} = 120x^{-1}$（人数と時間） |

: direct と inverse {#tbl-sl25-variation}

**$n = 2$ を「比例」とだけ書かないでください。** $y = 4.9x^{2}$ なら、「$y$ は $x^{2}$ に比例する」と、**何に比例するのか**まで書きます。
...
#### $b$ は「くり返しの速さ」です {#what-is-b}

**$b$ そのものは period ではありません。** ここを取り違える人が、とても多いところです。

$b$ が表しているのは、**どれだけ速くくり返すか**です。$b$ が大きいほど波は横につまり、$b$ が小さいほど横に伸びます。

@fig-sl25-sinusoid では $b = 30$ なので、
...
::: {.callout-important}
## $\sin$ でも $\cos$ でも、period は $\dfrac{360}{b}$
$$
...
$\sin$ と $\cos$ は**横にずれているだけ**で、くり返しの速さは同じです。ですから、**どちらで書かれていても period の求め方は変わりません。**

`Find the period` と聞かれて **$b$ の値をそのまま書いてしまう**のが、この項目で多い失点です。答えるのは $\dfrac{360}{b}$ のほうです。
:::
...
::: {.callout-important}
## Sinusoidal model で特に覚える3つ
**amplitude $= |a|$**、**period $= \dfrac{360}{b}$**、**principal axis: $y = d$** の $3$ つです。どれも Formula Booklet には載っていません。
...
**(b)** $k = 180 > 0$ で $0 < 0.94 < 1$ なので decay です（[第4節の表](#growth-decay)）。$t$ が大きくなると $180(0.94)^{t}$ は $0$ に近づき、残るのは $20$ です。
...
**(c)** $t = 30$ を電卓に入れます（[GDCの使い方](#read-values)）。
...
タンクを満たすのにかかる時間 $T$ 分は、使う蛇口の数 $n$ に**反比例**します（$n = -1$ の inverse variation）。モデルは $n \geq 1$ に対して $T = an^{-1}$ です。
...
どれも `lower bound?` と `upper bound?` を聞いてきます。**`enter` は $2$ 回**です（→ [SL 2.4](sl-2-4.qmd#bounds)）。
...
::: {.callout-tip collapse="true"}
## 解説
**(a)** $1.5$ は $0 < t \leq 2$ に入るので**上の式**です。$t$ が入っていないので、答えは $5$ ドルのままです。
...
$k = 5000 > 0$ で $1.06 > 1$ なので growth です（[第4節の表](#growth-decay)）。SL 1.4 の compound interest と同じ形で、$1.06$ は「年 $6\%$ 増える」を意味しています。
...
::: {.callout-tip collapse="true"}
## 解説
$n = 2$ で正なので、**direct variation** です。$d$ は **$t^{2}$ に比例**しています。$t$ が $2$ 倍になると $d$ は $4$ 倍になります。
...
- $y = ax^{n}$ の形に合う（$n = -1$ なら $xy$ が一定、$n = -2$ なら $yx^{2}$ が一定） → **direct / inverse variation**

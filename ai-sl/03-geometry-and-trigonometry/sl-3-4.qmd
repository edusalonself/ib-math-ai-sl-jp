...
- 扇形と三角形を組み合わせた形（**minor segment**・複合図形）の面積を求められる。
...
この分数を、円全体の長さ・面積に**掛けるだけ**です。以下が、公式です。
...
やることは、**式に数を入れてから、その方程式を解く**ことです。**GDC が使えるなら、`nSolve` で解くのが確実です**（[GDCの使い方](#nsolve)）。移項をしなくて済むので、途中で間違えません。
...
### 5. minor segment（小さい方の弓形）の面積 {#segment-area}

ここで扱うのは、中心角が $0^\circ < \theta \leq 180^\circ$ の場合です。このとき、弦で切り取られた $2$ つのうち**大きくないほう**が **minor segment**（小さい方の弓形）です（$\theta = 180^\circ$ なら、ちょうど半分ずつになります）。

minor segment の面積は、**扇形から三角形を引いて**出します。
...
$$
\text{minor segment} = \frac{\theta}{360}\times \pi r^{2} \; - \; \frac{1}{2}r^{2}\sin\theta
$$ {#eq-sl34-segment}

::: {.callout-note}
## major segment（大きい方）を聞かれたら
**円全体から minor segment を引きます。**

$$
\text{major segment} = \pi r^{2} - (\text{minor segment})
$$

@eq-sl34-segment の $\theta$ に $180^\circ$ より大きい角をそのまま入れる、というやり方もありますが、**引き算のほうが図で確かめやすい**です。
:::
...
![A sector with radius $12$ cm and angle $65^\circ$](img/sl-3-4-we1.svg){#fig-sl34-we1 width=72%}
...
![An arc of $20$ cm on a circle of radius $9$ cm](img/sl-3-4-we2.svg){#fig-sl34-we2 width=62%}
...
![A sector with angle $140^\circ$ and area $95$ cm$^2$](img/sl-3-4-we3.svg){#fig-sl34-we3 width=62%}
...
![The sector, the triangle and the minor segment](img/sl-3-4-we4.svg){#fig-sl34-we4 width=66%}
...
::: {.callout-tip}
## 引き算の答えは、扇形の面積を超えません
この minor segment は扇形の**一部分**なので、$20.6 < 69.8$ でなければおかしいです。
...
::: {.callout-warning}
## minor segment なのに、三角形を引き忘れる
`segment` と `sector` は、英語で1文字違いです。**問題文を読み飛ばすと取り違えます。**

`chord`（弦）という語が問題文にあれば、まず segment を疑ってください。

引き算をしたあとは、**答えが扇形の面積を超えていない**ことを確かめてください。`major segment` と書かれていたら、**円全体から引く**ほうです（[第5節](#segment-area)）。
:::
...
$\dfrac{65}{360} \times 2 \times \pi \times 12$
...
### 5. 逆算は `nSolve` が確実です {#nsolve}

$\theta$ や $r$ を求める問題は、**式に数を入れた形のまま**電卓に渡せます。移項をしなくて済みます。
...
$\texttt{nSolve}\left(20 = \dfrac{t}{360} \times 2 \times \pi \times 9,\ t,\ 0,\ 360\right) \ \to \ 127.323\ldots$

**半径を求める**（例題3）

$\texttt{nSolve}\left(95 = \dfrac{140}{360} \times \pi \times r^{2},\ r,\ 0,\ 100\right) \ \to \ 8.81805\ldots$

最後の2つの数は、**解を探す範囲**（lower bound と upper bound）です。$\theta$ なら `0, 360`、$r$ なら `0, 100` のように、**ありえない値を先に切っておきます**（SL 3.2 と同じ考え方です）。

::: {.callout-important}
## $r$ を求めるときは、範囲を `0,` から始めてください
$r^{2}$ の方程式には**負の解**もあります。範囲を切らないと、`nSolve` が $-8.818\ldots$ を返すことがあります。
...
電卓が返すのは数だけです。**cm なのか cm$^2$ なのかは、自分で書きます。** ここは落としやすいところです。

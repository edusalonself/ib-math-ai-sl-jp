...
法則は次の $3$ つで全部です。公式集の **AHL 1.9** の欄にも、この $3$ つが $\ \text{for } a,\ x,\ y > 0$ という条件つきで印刷されています（**条件は、下の囲みのとおり $a \neq 1$ を入れて $4$ つ**です）。**式そのものを覚える必要はありません。** 覚えるべきなのは、**どの場面でどれを使うか**のほうです。
...
## 使える条件は $a > 0$、$a \neq 1$、$x > 0$、$y > 0$ です
公式集の AHL 1.9 の欄には $\ \text{for } a,\ x,\ y > 0$ としか書かれていませんが、**$a \neq 1$ も必要**です。この条件は、公式集では [SL 1.5](../../ai-sl/01-number-and-algebra/sl-1-5.qmd#log-intro) の欄のほうに印刷されています。
...
のときです。理由は $\log$ の定義そのものです。$\log_a x$ は「$a$ を何乗したら $x$ になるか」でした。

- $x \le 0$ … $10^{\,\square}$ は正の数にしかなりません。だから $\log(-5)$ や $\log 0$ は**ありません**。
- $a \le 0$ … 底として使えません。
- $a = 1$ … $1$ は何乗しても $1$ のままなので、「何乗したら $x$ になるか」に答えが決まりません。

試験に出る底は $10$ か $e$ なので、$a$ の $2$ つの条件で実際に困ることはありません。**手が止まるのは、いつも $x > 0$、$y > 0$ のほうです。**
:::
...
::: {.callout-tip}
## $3$ つ目が、いちばん働きます
試験で点になるのは、圧倒的に @eq-ahl19-power です。**指数の中にいる未知数を、外に出せる**からです（[第3節](#solve)）。

$1$ つ目と $2$ つ目は、式を整理するときに使います。
:::

### 2. $\log$ でも $\ln$ でも、法則は同じです {#log-ln}
...
### 3. 使いどころ その1 — 指数の中の未知数を、外に出す {#solve}
...
### 4. 使いどころ その2 — 掛け算を足し算に変えて、大きさを測る {#scale}
...
法則が使えるのは、$\log$ の中身が**掛け算・割り算・累乗**のときだけです。次の $3$ つは、**どれも成り立ちません。**
...
これで、[第5節](#not-laws)の $3$ つが成り立たない理由も見えます。$10^{\,p} + 10^{\,q}$ を $10$ の $1$ つの累乗にまとめる指数法則は、**存在しない**からです。
::::
...
`Find the exact value` なので、**電卓の小数近似ではなく、正確な値で答えます。** この例題では、$\log$ をまとめると中身が $10$ の整数乗になるので、答えも整数になります。$\log$ を $1$ つにまとめて、$10$ の何乗かを見るのが手順です。
...
ただし、`exact value` は「**小数近似にしない**」という意味であって、「整数になる」という意味ではありません。**問題によっては、分数・根号・$\pi$・$\log 12$ のような形が、そのまま正確な答えとして残ります。** この例題は、まとめた中身が $10$ の整数乗になるように作られている、というだけのことです。
:::
:::
...
[第3節](#solve)の $4$ 段そのままです。まず $3$ で割って、指数のかたまりだけにします。
...
::: {.callout-tip}
## 「整数部分 $+\ 1$」で覚えて構いません
$N$ が正の整数のとき、$\log_{10} N$ の整数部分が $30$ なら $N$ は $31$ 桁です（[第4節](#digits)）。
...
数を入れた反例は[第5節](#not-laws)にあります。**$\log$ の中が $+$ か $-$ なら、手は出せません。**
:::
...
$x = -2$ を入れると $\log(-5)$ になり、**存在しません**（[第1節](#conditions)）。**$\log$ の中身が正になるかを、必ず戻して確かめてください。**
:::

::: {.callout-warning}
## 桁数を、$\log$ の整数部分そのままで答える
$\log 2^{100} = 30.1$ から「$30$ 桁」と答える誤りです。正しくは **$31$ 桁**——**整数部分 $+\ 1$** です（[第4節](#digits)）。
:::
...
### 2. `nSolve` を使ってもよい {#gdc-solve-cmd}

数値だけが欲しいなら、`nSolve` でも出ます。

- `menu → Algebra → Numerical Solve`
- $\texttt{nSolve}\left(3 \times 1.4^{t} = 20,\ t\right)$
...
::: {.callout-tip collapse="true"}
## 解説
@exm-ahl19-single と同じ $2$ つの手順です。**係数を中へ入れる → 足すなら掛ける、引くなら割る。**
...
::: {.callout-tip collapse="true"}
## 解説
**(a)** @exm-ahl19-exp と同じ $4$ つの手順です。まず $80$ で割って $(1.06)^{n} = 6.25$、両辺の $\log$、$n$ が前に出る、割る。
...
$x = y = 10$ が選ばれているのは、$\log 10 = 1$ なので**右辺が暗算で $2$ と出る**からです（[第5節](#not-laws)）。
:::
...
これが AHL 2.10 の **log-log graph**（両対数グラフ）の考え方です（[第4節](#link-210)）。曲線のデータを $\log$ で直線に直してから、傾きを読んで $n$ を決める、という使い方をします。
:::

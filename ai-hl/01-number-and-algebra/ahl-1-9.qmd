...
法則は @fig-ahl19-three の $3$ つで全部です。公式集の **AHL 1.9** の欄にも、この $3$ つが $\ \text{for } a,\ x,\ y > 0$ という条件つきで印刷されています（**条件は、下の囲みのとおり $a \neq 1$ を入れて $4$ つ**です）。**式そのものを覚える必要はありません。** 覚えるべきなのは、**どの場面でどれを使うか**のほうです。
...
::: {.callout-warning}
## 底の条件は $a > 0$ と $a \neq 1$ の $2$ つです
公式集の AHL 1.9 の欄には $\ \text{for } a,\ x,\ y > 0$ としか書かれていませんが、**$a \neq 1$ も必要**です。$1$ は何乗しても $1$ のままなので、$\log_1 x$ という道具は作れません。この条件は、公式集では [SL 1.5](../../ai-sl/01-number-and-algebra/sl-1-5.qmd#log-intro) の欄のほうに $a \neq 1$ と印刷されています。
...
のときです（[第2節](#conditions)）。試験に出る底は $10$ か $e$ なので、$a$ の条件で実際に困ることはありません。**気をつけるのは $x > 0$、$y > 0$ のほうです。**
:::
...
公式集の AHL 1.9 の欄には、$3$ つの式の下に **for $a,\ x,\ y > 0$** と印刷されています。$a \neq 1$ はそこには書かれていませんが、**SL 1.5 の欄**に印刷されていて、AHL 1.9 でもそのまま必要です（@eq-ahl19-conditions）。

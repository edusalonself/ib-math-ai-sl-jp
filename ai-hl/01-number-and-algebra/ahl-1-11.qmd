...
しかし、実際には $1$ を超えません。$\dfrac{1}{2}$ を足すと $\dfrac{1}{2}$、次で $\dfrac{3}{4}$、次で $\dfrac{7}{8}$——**残りをいつも半分ずつ埋めていく**ので、$1$ に近づきはしても追い越せません。
...
これが @eq-ahl111-sinf です。**公式集の $\lvert r \rvert < 1$ という条件は、$r^{n} \to 0$ が成り立つ範囲**を書いたものです。
...
シラバスの Guidance が、$3$ つの行き先を挙げています。

- **fractal**（フラクタル）… [AHL 3.9](../03-geometry-and-trigonometry/ahl-3-9.qmd#fractals) で、縮小をくり返して作る図形。長さや面積の合計が無限級数になります
- **Markov chain** … [AHL 4.19](../04-statistics-and-probability/ahl-4-19.qmd) の long-term の状態。同じ「落ち着き先」の考え方です
- **limit**（極限）… [SL 5.1](../../ai-sl/05-calculus/sl-5-1.qmd) の $x \to a$ と同じ読み方です

Connections 欄の**弾むボール**が、いちばんよく出る文脈です（@exm-ahl111-ball）。
...
$18$ と返ります。**分母全体をかっこで囲む**ことだけ気をつけてください。`12/1-1/3` は $12 - \dfrac{1}{3}$ になります。

### 2. 部分和で、近づく様子を見る {#gdc-partial}

$\Sigma$ のテンプレート（$9$ キーの右のパレット）を使うと、部分和を一度に出せます。下の箱に `n=1`、上の箱に足す個数、まん中に `12×(1/3)^(n-1)` を入れます（[SL 1.3](../../ai-sl/01-number-and-algebra/sl-1-3.qmd) と同じ操作です）。

```
sum(seq(12×(1/3)^(n-1), n, 1, 10))
```
...
**$\lvert r \rvert < 1$ の確認は、自分でやる仕事**です（[第6節](#diverge)）。
:::

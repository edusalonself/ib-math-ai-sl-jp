...
$i^{4} = 1$ に戻るので、ここから先は同じ $4$ つの繰り返しです。

![What the discriminant tells you, and the four-step cycle of $i$](img/ahl-1-12a-disc.svg){#fig-ahl112a-disc width=100%}

@fig-ahl112a-disc の (b) がその輪です。ですから $i$ の大きな累乗は、**指数を $4$ で割った余り**だけ見れば決まります。

$$
i^{23} = i^{20} \cdot i^{3} = (i^{4})^{5} \cdot i^{3} = 1 \cdot (-i) = -i
$$

複素数そのものの累乗（$(3+2i)^{7}$ のようなもの）については、シラバスは技術を使うよう書いています。
...
@fig-ahl112a-disc の (a) は、$y = x^{2}-4x+3$、$y = x^{2}-4x+4$、$y = x^{2}-4x+5$ の $3$ つです。形は同じで、上下にずれているだけです。
...
**係数が実数であることが効いています。** 係数に $i$ が混じっている方程式では、解が組になるとはかぎりません。
...
虚数単位の $i$ は、**記号パレット**から選びます。TI 公式のガイドでは、handheld で `ctrl` と `k` を押すとパレットが開く、と書かれています。

::: {.callout-warning}
## キーボードの `i` は、ふつうの変数です
アルファベットとして打った `i` は、値の入っていない**変数**として扱われます。非CAS の CX II は値の入っていない変数を計算できないので、そこで止まってエラーの表示が出ます。
...
`enter` で入力すると $-\dfrac{7}{26} + \dfrac{17}{26}i$ と**分数のまま**、`ctrl + enter` で入力すると小数で返ります（[AHL 1.14](ahl-1-14.qmd) の逆行列と同じ挙動です）。
...
- 商 $\dfrac{z_1}{z_2} = w$ を出したら、`w × z2` を計算して $z_1$ に戻るか見る
- $2$ 次方程式の解 $x_0$ を出したら、`2x₀^2 + 2x₀ + 5` を計算して $0$ になるか見る

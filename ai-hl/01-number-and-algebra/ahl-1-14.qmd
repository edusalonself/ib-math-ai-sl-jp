...
`det(` は `menu → Matrix & Vector` の中の `Determinant` からも入れられます。
...
@fig-ahl114-gdc は、$\begin{pmatrix} 1 & 2 \\ 3 & 4 \end{pmatrix}$ を `a` に保存してから、$a^{-1}$ と $\texttt{det}(a)$ を打った画面です。**分数のまま返ってきます。**
...
$a \times a^{-1}$ と打って $I$ が出れば、逆行列は合っています。
:::

...
**公式集の $5.12$ の欄にあります。配られるので、覚える必要はありません。** 見出しは「**Volume of revolution about $x$ or $y$-axes**」で、$y$ 軸のための式（[第3節](#y-axis)）と $2$ つ並んでいます。

面積の式には modulus（絶対値）が付いていましたが（[AHL 5.12a](ahl-5-12a.qmd#idea)）、**体積の式には付いていません。** $y$ が負でも $y^{2}$ は正になるので、**$2$ 乗が絶対値の代わり**をしています（[第2節](#no-abs)）。

![How a volume of revolution is built up](img/ahl-5-12b-disc.svg){#fig-ahl512b-disc width=70%}

@fig-ahl512b-disc を見てください。回転体を、軸に垂直な薄い **disc**（円板）に切り分けます。
...
@fig-ahl512b-disc の赤い部分が、その $1$ 枚です。
...
![A check: rotating a line gives a cone](img/ahl-5-12b-cone.svg){#fig-ahl512b-cone width=70%}

@fig-ahl512b-cone がその絵です。$x = 6$ のとき $y = 3$ なので、半径 $3$、高さ $6$ の円錐です。
...
**(b)** @fig-ahl512b-cone を見てください。$x = 6$ のとき $y = 3$ です。
...
積分のテンプレートが出るので、**画面に出てくる形のまま**入れます。
...
::: {.callout-warning}
## $\pi$ は専用のキーで入れてください
`3.14` と打つと、丸めた値で計算が進みます（[AHL 5.9b](ahl-5-9b.qmd#gdc-rates)）。キーボードの $\pi$ のキーを使ってください。

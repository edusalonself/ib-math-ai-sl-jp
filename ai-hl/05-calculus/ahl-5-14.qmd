...
**Formula booklet に 5.14 の欄はありません。** AI HL の公式集の Topic 5 は、**5.13 の次が 5.16** です。変数分離のやり方は書かれていないので、**やり方そのものを覚える必要があります。** 使う積分のほうは載っていて、$\dfrac{1}{x}$ と $e^x$ は **5.11 の Standard integrals** の欄、べき乗のほうは **5.5** の欄です。

新しいのは、**ことばから式を立てること**（[第2節](#setup)）と、**その式を積分で解くこと**（[第4節](#separate)）の $2$ つだけです。計算そのものは [SL 5.3](../../ai-sl/05-calculus/sl-5-3.qmd) と [SL 5.5](../../ai-sl/05-calculus/sl-5-5.qmd)、それに [AHL 5.11a](ahl-5-11a.qmd) の積分で足ります。
...
**覚えてもよいかもしれません。** 何度も出てきます。

::: {.callout-tip}
## $k$ の符号が、増えるか減るかを決めます（$A > 0$ のとき）
| $k$ | $y = Ae^{kx}$ の形（$A > 0$） |
|---|---|
...
**$A$ の符号にも気をつけてください。** $A < 0$ なら曲線は $x$ 軸の**下側**にあり、増減の向きが逆になります。$A = 0$ なら $y = 0$ が恒等的に成り立ちます。文脈問題では量が正なので $A > 0$ になりますが、**式だけを与えられたときは、$A$ の符号を確かめてから**増減を言ってください（演習 $4$）。
...
$P = Ae^{kt}$ は、$A > 0$ かつ $k > 0$ ならどこまでも増えます（個体数のモデルなので $A > 0$ です）。**現実には、そうはなりません。**
...
$\dfrac{1}{\sqrt{G}} = G^{-\frac{1}{2}}$ なので、べき乗の積分の公式が使えます（[SL 5.5](../../ai-sl/05-calculus/sl-5-5.qmd#rule)）。

$$
\int G^{-\frac{1}{2}}\, dG = \int k\, dt
$$

$$
\frac{G^{\frac{1}{2}}}{\tfrac{1}{2}} = kt + C
$$
...
$\ln$ になるのは $\dfrac{1}{G}$ のときだけです。$\dfrac{1}{\sqrt{G}} = G^{-\frac{1}{2}}$ は、**指数が $-1$ ではない**のでべき乗の公式を使います。

$$
\int G^{-\frac{1}{2}}\, dG = \frac{G^{\frac{1}{2}}}{1/2} = 2\sqrt{G}
$$
:::
...
$\ln$ になるのは $\dfrac{1}{y}$ だけです。$\dfrac{1}{\sqrt{y}} = y^{-\frac{1}{2}}$ はべき乗の公式です（@exm-ahl514-algae）。
:::
...
電卓は、**答えを確かめるため**に使ってください。解を微分して $1$ 点で合わせる確かめ方は、[AHL 5.9a](ahl-5-9a.qmd#gdc-deriv) と同じです。
:::
...
**(b)** [Given that the constant of integration $A$ satisfies $A > 0$, describe the behaviour of $y$ as $x$ increases when (i) $k > 0$ and (ii) $k < 0$.]{.q-en}
...
**(b)** 積分定数 $A$ が $A > 0$ を満たすとして、$x$ が増えるときの $y$ のふるまいを、(i) $k > 0$ のとき、(ii) $k < 0$ のときについて述べなさい。
...
::: {.model-answer}
*Assuming $A > 0$: when $k > 0$ the exponent $kx$ increases as $x$ increases, so $y$ increases without limit. This is exponential growth.*
:::

（$A > 0$ とすると、$k > 0$ のとき $x$ が増えると指数 $kx$ も増えるので、$y$ は限りなく増える。これは指数的増加である、という意味です。）

**(b) (ii)**

::: {.model-answer}
*Assuming $A > 0$: when $k < 0$ the exponent $kx$ decreases as $x$ increases, so $y$ tends towards $0$ but never reaches it. This is exponential decay.*
:::

（$A > 0$ とすると、$k < 0$ のとき $x$ が増えると指数 $kx$ は減るので、$y$ は $0$ に近づくが決して $0$ にはならない。これは指数的減衰である、という意味です。）
:::
...
この問題は $A > 0$ と指定してあります。**$A$ の符号で話が変わる**からです。

- $A > 0$ … いまの説明のとおりの exponential growth ／ decay
- $A < 0$ … 曲線は $x$ 軸の**下側**にあり、増減の向きが逆になります（$k > 0$ なら限りなく減り、$k < 0$ なら下から $0$ に近づきます）
- $A = 0$ … $y = 0$ が恒等的に成り立ちます（increasing でも decreasing でもありません）

答案でも、$A > 0$ を前提に書くときは ***Assuming* $A > 0$ の $1$ 語を添える**のが安全です。
:::
...
**(b) では、丸める前の $k$ を使ってください。**$0.101$ で計算すると $44.2$ となり、**$3$ 桁目がずれます。**
...
**(b)** 丸める前の $k$ で計算します。$40e^{5k} = 40 \times 4^{\frac{5}{3}} = 403.17\ldots$ で、人数なので $403$ 人です。

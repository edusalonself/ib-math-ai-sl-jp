...
- 異なる $2$ つの実数の固有値のとき、**厳密解**を書き、初期条件から定数を決められる（[AHL 5.17a](ahl-5-17a.qmd)）。
...
これまでのページで扱ってきたのは、**1階**（**first order**）の微分方程式でした（[AHL 5.16b](ahl-5-16b.qmd) の Euler 法、[AHL 5.17a](ahl-5-17a.qmd) の厳密解と phase portrait）。$\dfrac{dx}{dt}$ までしか出てこない式のことです。このページで出てくるのは、$\dfrac{d^{2}x}{dt^{2}}$ が入った**2階**（**second order**）の式です。
...
ここで $f$ は、**微分方程式の右辺**です（[AHL 5.16a](ahl-5-16a.qmd#formula)）。つまり $f(x_n,\ t_n)$ が $\dfrac{dx}{dt}$ の値です。

$$
\frac{dx}{dt} = f(x,\ t)
$$

**Euler 法は、この形に書けて初めて使えます。** ところが @eq-ahl518-spring が教えてくれるのは $\dfrac{d^{2}x}{dt^{2}}$ のほうで、**$\dfrac{dx}{dt} = (\text{$x$ と $t$ の式})$ の形には書けません。**
...
**$2$ 本目**は、もとの2階の式です。$\dfrac{d^{2}x}{dt^{2}}$ は「$\dfrac{dx}{dt}$ の微分」、つまり **$y$ の微分 $\dfrac{dy}{dt}$** ですから
...
左の $f\!\left(x,\ \dfrac{dx}{dt},\ t\right)$ は、**$\dfrac{d^{2}x}{dt^{2}} = \ldots$ と直したときの右辺**という意味です。$x$ と $\dfrac{dx}{dt}$ と $t$ が入っていてよい、ということを表しているだけで、特別な関数ではありません。@eq-ahl518-spring なら $-2x - 3\dfrac{dx}{dt}$ がその中身です。

**これで、$x$ と $y$ の連立1階になりました。** ここでの **$t$ は時刻**で、$x$（位置）も $y$（速度）も $t$ の関数です——[AHL 5.16b](ahl-5-16b.qmd) や [AHL 5.17a](ahl-5-17a.qmd) の連立系と、まったく同じ形です。ここから先は、その $2$ ページの内容です。シラバスも、この書きかえを名指ししています。

> Write as coupled first order equations $\dfrac{dx}{dt} = y$ and $\dfrac{dy}{dt} = f(x,\ y,\ t)$.

**新しい公式はありません。** 公式集に 5.18 の欄は無く、使うのは**5.16 の欄**（連立系の Euler 法）と**5.17 の欄**（連立系の厳密解）です。ただし、$\det(M - \lambda I) = 0$ は**公式集に載っていない**ので、覚えておいてください（[AHL 5.17b](ahl-5-17b.qmd)）。

#### 書きかえの手順は $3$ つです {#steps}

1. $\dfrac{dx}{dt}$ に **$y$ という名前を付ける**。その約束をそのまま、**$1$ 本目の式** $\dfrac{dx}{dt} = y$ として書く
...
です（$\dfrac{d^{2}x}{dt^{2}} = -bx - a\dfrac{dx}{dt}$ と直して、$\dfrac{dx}{dt}$ を $y$ にしただけです）。

右辺はどちらも **$x$ と $y$ の一次式だけ**で、定数項も $t$ もありません。これは [AHL 5.17a](ahl-5-17a.qmd#idea) が扱った形そのものです。ですから、そのまま行列に書けます。

$$
\frac{d\mathbf{X}}{dt} = M\mathbf{X}, \qquad
\mathbf{X} = \begin{pmatrix} x \\ y \end{pmatrix}, \qquad
M = \begin{pmatrix} 0 & 1 \\ -b & -a \end{pmatrix}
$$ {#eq-ahl518-matrix}

**なぜこの $M$ でよいのか**は、右辺をかけ算してみると分かります。

$$
M\mathbf{X} = \begin{pmatrix} 0 & 1 \\ -b & -a \end{pmatrix}\begin{pmatrix} x \\ y \end{pmatrix}
= \begin{pmatrix} 0 \cdot x + 1 \cdot y \\ (-b) \cdot x + (-a) \cdot y \end{pmatrix}
= \begin{pmatrix} y \\ -bx - ay \end{pmatrix}
$$

左辺は $\dfrac{d\mathbf{X}}{dt} = \begin{pmatrix} \frac{dx}{dt} \\[1mm] \frac{dy}{dt} \end{pmatrix}$ です。**上どうし、下どうしをくらべる**と

$$
\frac{dx}{dt} = y, \qquad \frac{dy}{dt} = -bx - ay
$$

となって、書きかえた $2$ 本の式に戻りました ✓
...
### 5. 固有値が異なる $2$ つの実数なら、厳密解が書けます {#exact}

**この節ですること：行列にしたあとを、[AHL 5.17a](ahl-5-17a.qmd) の手順どおりに解きます。**

@eq-ahl518-spring の $\dfrac{d^{2}x}{dt^{2}} + 3\dfrac{dx}{dt} + 2x = 0$ で、順にやってみます。おもりは $x = 1$ の位置から、静かに手をはなすとします。

#### 手順1. 行列にする

$a = 3$、$b = 2$ なので（[第4節](#matrix)）
...
**異なる $2$ つの実数**になりました。ですから、厳密解が書けます。

#### 手順3. 固有ベクトルを求める

この形の $M$ では、$\mathbf{p} = \binom{1}{\lambda}$ と決まっています（下のヒント）。
...
**$\lambda$ を求めたら、eigenvector はもう書けています。**
:::

#### 手順4. 一般解を書く

[AHL 5.17a](ahl-5-17a.qmd#general) の公式に入れるだけです。

$$
\mathbf{X} = A\binom{1}{-1}e^{-t} + B\binom{1}{-2}e^{-2t}
$$

成分で書くと、こうなります。

$$
x = Ae^{-t} + Be^{-2t}, \qquad y = -Ae^{-t} - 2Be^{-2t}
$$

**$y$ は $x$ を微分したもの**になっています ✓ ここが検算です（@eq-ahl518-sub）。

#### 手順5. 初期条件で $A$ と $B$ を決める

初期条件は **$2$ つ**要ります（[第2節](#convert)）。$x = 1$ から静かに手をはなすので、$x(0) = 1$、$y(0) = 0$ です。
...
**$t$ が大きくなると $x \to 0$** です。おもりは、行き過ぎずにつりあいの位置へ戻ります。

この $5$ 段階を**答案としてどう書くか**は、@exm-ahl518-exact を見てください。
...
::: {.callout-warning}
## この課程で厳密解を求めるのは、固有値が異なる $2$ つの実数のときだけです
...
![What the eigenvalues say about the motion](img/ahl-5-18-motion.svg){#fig-ahl518-motion width=100%}

| $a$ と $b$ | 固有値 | 動き方 | 図 |
|---|---|---|---|
| $a > 0$、$b > 0$、$\Delta > 0$ | 異なる $2$ つの実数、**両方負** | **振動せずに** $0$ へ戻る | 左 |
| $a > 0$、$b > 0$、$\Delta < 0$ | 複素数、**実部が負** | **振動しながら** $0$ へ戻る | 中（[AHL 5.17b](ahl-5-17b.qmd)） |
| $a = 0$、$b > 0$ | **純虚数** | **振動が続く**（大きさが変わらない） | 右（[AHL 5.17b](ahl-5-17b.qmd)） |
| $a < 0$、$b > 0$、$\Delta < 0$ | 複素数、**実部が正** | **振動しながら大きくなる** | [AHL 5.17b](ahl-5-17b.qmd) |
| $b < 0$ | 異なる $2$ つの実数、**異符号** | ほとんどの出発点で**離れていく**（saddle point） | [AHL 5.17a](ahl-5-17a.qmd#saddle) |
...
- $a = 0$ … じゃまがないので、**いつまでも揺れ続けます**（@fig-ahl518-motion の右）
- $0 < a < 2\sqrt{b}$（$\Delta < 0$）… **揺れながら、だんだんおさまります**（中）
- $a > 2\sqrt{b}$（$\Delta > 0$）… **揺れる前におさまります**（左）
...
![The same three motions in the $(x, y)$ plane](img/ahl-5-18-phase.svg){#fig-ahl518-phase width=100%}

@fig-ahl518-phase は、@fig-ahl518-motion と同じ $3$ つの動きを $(x,\ y)$ 平面に描いたものです。左が [AHL 5.17a](ahl-5-17a.qmd#sketch)、中と右が [AHL 5.17b](ahl-5-17b.qmd) と同じ絵ですが、**この項目では縦軸に意味があります。**
...
**異なる $2$ つの実数**なので、厳密解が書けます（[第5節](#exact)）。
...
（不要である。厳密解が求められるのは固有値が異なる $2$ つの実数のときだけで、ここでは複素数なので、運動の定性的な説明だけでよい、という意味です。）
...
1. **$y$ が $x$ の導関数になっているか。** 出した $x$ を微分して、$y$ と一致するか見ます（[AHL 5.9a](ahl-5-9a.qmd#gdc-deriv)）

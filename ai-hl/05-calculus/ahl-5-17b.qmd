...
違うのは、**characteristic equation の解が実数にならない**場合です。
...
先に [AHL 5.17a](ahl-5-17a.qmd) を読んでください。**固有値の出し方は同じ**です。
:::
...
公式集の **5.17** の欄には $\mathbf{x} = A\mathbf{p}_1 e^{\lambda_1 t} + B\mathbf{p}_2 e^{\lambda_2 t}$ が載っていますが、**使うのは [AHL 5.17a](ahl-5-17a.qmd) だけ**です。

いっぽう $\det(M - \lambda I) = 0$ は**公式集に載っていません**（[AHL 1.15](../01-number-and-algebra/ahl-1-15.qmd#char)）。**このページでも使うので、覚えてください。**
:::

::: {.callout-warning}
## [AHL 1.15](../01-number-and-algebra/ahl-1-15.qmd) との違いに注意してください
[AHL 1.15](../01-number-and-algebra/ahl-1-15.qmd) では、**対角化（diagonalization）**を扱うので、シラバスが「実数で異なる固有値」に限定していました。ですから、あのページでは複素数の固有値は出てきません。

**この項目では出てきます。** ただし、**固有値を求めるところまで**です。固有ベクトルを複素数で求めることは、AI HL では求められません。
...
![Four pictures for complex eigenvalues](img/ahl-5-17b-cases.svg){#fig-ahl517b-cases width=100%}

@fig-ahl517b-cases の (a) と (b) を見くらべてください。**どちらも回っています**が、片方は内へ、もう片方は外へ向かっています。

| 実部 | 動き |
|---|---|
| **負**（$a < 0$） | 原点へ**近づく**（spiral in） |
| **正**（$a > 0$） | 原点から**離れる**（spiral out） |
| **$0$**（純虚数） | どちらでもない。**閉じた曲線** |
...
@fig-ahl517b-cases の (c) と (d) が、その $2$ つです。

::: {.callout-important}
## 円になるとはかぎりません
(c) と (d) は、**固有値が同じ $\pm 2i$** です。それでも、片方は円、もう片方は楕円です。
...
::: {.callout-tip}
## 判定の順番
1. **判別式**を見る（$(\text{対角の和})^2 - 4\det M$）
   - 正なら実数 → [AHL 5.17a](ahl-5-17a.qmd) の $3$ つ
   - 負なら複素数 → このページ
2. 複素数なら、**実部の符号**を見る
...
@fig-ahl517b-cases の (a) が、この系の絵です。
...
**検算。** $x^{2} + y^{2}$ が一定ということは、原点からの距離が変わらないということです。**この系では、軌道は円**になります ✓ @fig-ahl517b-cases の (c) と一致します。
...
固有値が $\pm 2i$ の系には、円になるものも楕円になるものもあります（@fig-ahl517b-cases の (c) と (d)）。ですから、(b) の答えは `circles or ellipses` です。
:::
...
**(c)** 向きは $(1,\ 0)$ の代入で決めます（[第4節](#direction)）。左下の数が $+3$ なので反時計回りです。@fig-ahl517b-cases の (a) と同じ形の絵になります。
...
[8]{.ex-no} [@fig-ahl517b-cases parts (c) and (d) both come from systems with eigenvalues $\pm 2i$, yet the pictures are different.]{.q-en}
...
@fig-ahl517b-cases の (c) と (d) は、どちらも固有値が $\pm 2i$ の系から来ていますが、絵は違います。

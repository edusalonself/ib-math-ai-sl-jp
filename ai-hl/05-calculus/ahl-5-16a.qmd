...
| 方法 | 出てくるもの | 使えるとき |
|---|---|---|
| [AHL 5.14](ahl-5-14.qmd) 変数分離 | **正確な式** | 変数分離できる式のときだけ |
...
::: {.callout-important}
## 公式集に載っています（5.16 の欄）
[AHL 5.14](ahl-5-14.qmd) と [AHL 5.15](ahl-5-15.qmd) には公式集の欄がありませんでしたが、**5.16 にはあります。**
...
**この式を覚える必要はありません。** 試験で配られます。

覚えておくのは、**式の意味と、表の作り方**です。同じ欄には、[AHL 5.16b](ahl-5-16b.qmd) で使う**連立系の式**も並んでいます。
:::
...
::: {.callout-important}
## 折れ線なので、本当の曲線からはずれます
@fig-ahl516a-step の (d) を見てください。本当の解は曲がっているのに、Euler は**まっすぐ**進みます。
...
つまり、**誤差は歩くほどたまります。** @fig-ahl516a-step の (d) で、右へ行くほど差が開いているのは、そのためです。
...
矢印は `ctrl` + `var` です（[AHL 5.14](ahl-5-14.qmd#gdc-value)）。
...
2. **slope field** を描いて、折れ線が場に沿っているか（[AHL 5.15](ahl-5-15.qmd#gdc-plot)。ただし試験モードでは使えません）
3. **$h$ を半分**にして、答えが大きく変わらないか

::: {.callout-tip}
## $h$ を半分にして比べるのが、いちばん実用的です
...
**この形は [AHL 5.14](ahl-5-14.qmd) でも「式を立てるところまで」とされていました。** 数値で解くのが、まさにこの項目の仕事です。

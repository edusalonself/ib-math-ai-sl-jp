...
::: {.callout-important}
## 公式集に 5.10 の欄はありません
公式集の Topic 5（HL）には、$5.3$、$5.9$、$5.5$、$5.8$、$5.11$、$5.12$、$5.13$、$5.16$、$5.17$ の欄がありますが、**$5.10$ の欄はありません。**

ですから、この項目で**覚える必要があるもの**が $2$ つあります。

$$
f'(a) = 0 \ \text{のとき} \qquad f''(a) < 0 \ \Rightarrow \ \text{maximum} \qquad f''(a) > 0 \ \Rightarrow \ \text{minimum}
$$ {#eq-ahl510-test}

**$f'(a) = 0$ が前提です。** stationary point（停留点、$f'$ が $0$ になる点）を先に見つけてから使う道具で、どこでも使えるわけではありません（[第2節](#test)）。

$$
f''(x) > 0 \ \Rightarrow \ \textbf{concave-up} \qquad\qquad f''(x) < 0 \ \Rightarrow \ \textbf{concave-down}
$$ {#eq-ahl510-concave}

**この $2$ 行だけです。** どちらも「$f''$ の符号を見る」という同じことを、別の場面で言っているだけです（[第3節](#concavity)）。
:::

::: {.callout-note}
## SL で「使いません」と書いた $f''$ が、ここで出ます
[SL 5.6](../../ai-sl/05-calculus/sl-5-6.qmd) には、**「second derivative（$2$ 階導関数）は、AI SL では使いません」**と書きました。AI SL のシラバスに $f''$ が入っていないからです。

**HL では、その $f''$ が出ます。** SL では stationary point を $f'$ の符号の変化で見分けていましたが（[SL 5.6 第2節](../../ai-sl/05-calculus/sl-5-6.qmd#sign)）、HL では **$1$ 回の代入で決められる**やり方が加わります。

とはいえ、新しい微分の公式は出てきません。使うのは [SL 5.3](../../ai-sl/05-calculus/sl-5-3.qmd) と [AHL 5.9a](ahl-5-9a.qmd)・[AHL 5.9b](ahl-5-9b.qmd) の公式だけです。**やることは「もう $1$ 回微分する」だけ**です。
:::
...
### 2. second derivative test — 山か谷かを $1$ 回の代入で決めます {#test}

[SL 5.6](../../ai-sl/05-calculus/sl-5-6.qmd#stationary) では、$f'(x) = 0$ を解いて stationary point（停留点）を出したあと、**その前後で $f'$ の符号がどう変わるか**を調べて maximum か minimum かを決めました。停留点 $1$ つにつき、左右の値を入れて調べる必要がありました。
...
![The sign of the second derivative at a stationary point](img/ahl-5-10-test.svg){#fig-ahl510-test width=100%}

@fig-ahl510-test の (a) は、$f(x) = x^{3} - 3x^{2} - 9x + 5$ を、$f'$ と $f''$ といっしょに描いたものです。
...
#### 符号の向きを取り違えないために {#which-way}

$f'' < 0$ が maximum で、$f'' > 0$ が minimum です。**逆に覚えている生徒がとても多い**ところです。

@fig-ahl510-test の (b) を見てください。**曲線の形そのもの**を覚えると、間違えなくなります。
...
#### $f''(a) = 0$ のときは、この test では決まりません {#fails}

**これがいちばん大事な注意です。**

$f'(a) = 0$ かつ $f''(a) = 0$ のとき、その点が maximum なのか minimum なのか、**second derivative test では何も分かりません。**

$2$ つの例を並べます。どちらも $x = 0$ で $f' = 0$、$f'' = 0$ です。
...
**同じ「$f'' = 0$」から、違う結論が出ています。** ですから、この場合は

$$
f''(a) = 0 \quad \Longrightarrow \quad \textbf{SL 5.6 の方法に戻る}
$$

つまり、**$a$ の前後で $f'$ の符号がどう変わるか**を調べてください（[SL 5.6 第2節](../../ai-sl/05-calculus/sl-5-6.qmd#sign)）。

::: {.callout-warning}
## 「$f'' = 0$ だから point of inflexion」とは書けません
$f''(a) = 0$ は、$a$ が point of inflexion であることの**保証にはなりません。** $f(x) = x^{4}$ が $x = 0$ で $f'' = 0$ なのに minimum である、というのがその例です（@tbl-ahl510-fails）。

**$f'' = 0$ で分かるのは「この test が使えない」ということだけ**です。

（**point of inflexion**（変曲点）そのものについては、[第5節](#inflexion)で説明します。）
:::

### 3. concave-up と concave-down {#concavity}

いままでは **stationary point での** $f''$ の符号を見ました。ここからは、**stationary point でなくても** $f''$ の符号に意味がある、という話です。
...
$$
f''(x) > 0 \ \text{の区間} \quad \Rightarrow \quad \textbf{concave-up}（下に凸）
$$

$$
f''(x) < 0 \ \text{の区間} \quad \Rightarrow \quad \textbf{concave-down}（上に凸）
$$
...
ですから **$x > 1$ で concave-up、$x < 1$ で concave-down** です。@fig-ahl510-test の (a) で、曲線が $x = 1$ を境に形を変えているのが見えます。

### 4. $f'$ と $f''$ の $4$ つの組み合わせ {#four}
...
| $f'$ | $f''$ | 形 | 読み方 |
|---|---|---|---|
| $+$ | $+$ | concave-up | 増えていて、**増え方も速くなっている** |
| $+$ | $-$ | concave-down | 増えているが、**増え方は鈍ってきている** |
| $-$ | $-$ | concave-down | 減っていて、**減り方が速くなっている** |
| $-$ | $+$ | concave-up | 減っているが、**減り方は鈍ってきている** |

: $f'$ と $f''$ を合わせて読む {#tbl-ahl510-four}

**試験で点になるのは、この表の右の列**です。数値を出したあと、$2$ 行目や $4$ 行目の言い方で英語を $1$ 文書けるかどうかで差がつきます。

::: {.callout-tip}
## 「増えている」と「増え方が増している」は別です
$f' > 0$ でも $f'' < 0$ なら、**増えてはいるが、頭打ちに向かっている**という意味です。

たとえば「新製品の売上は伸びているが、伸びは鈍ってきている」という状況が、ちょうどこれです（@tbl-ahl510-four の $2$ 行目）。

`Comment on` や `Interpret` では、**$2$ つを分けて書いてください。**
:::

### 5. concavity が変わる点 {#inflexion}
...
### 6. kinematics とのつながり {#kinematics}

シラバスは、この項目に次のリンクを付けています。

> Link to: kinematics (AHL5.13) and second order differential equations (AHL5.18).

**この節は、[AHL 5.13](ahl-5-13.qmd) を読む前でも後でも構いません。** kinematics の言葉そのものは、そちらのページで説明します。

[AHL 5.13](ahl-5-13.qmd#idea) の

$$
a = \frac{dv}{dt} = \frac{d^{2}s}{dt^{2}}
$$

は、**変位 $s$ の second derivative** です。つまり **acceleration は second derivative そのもの**です。

| このページの言い方 | kinematics での言い方 |
|---|---|
| $f'$ | velocity $v = \dot{s}$ |
...
: 同じものを $2$ つの言葉で {#tbl-ahl510-kin}

[AHL 5.18](ahl-5-18.qmd) の $\dfrac{d^{2}x}{dt^{2}}$ も、同じ second derivative です。**このページの記号が読めれば、そのまま読めます。**
...
$f''$ は **$f'$ を微分したもの**です。ですから $f''$ の符号は、**$f'$ が増えているか減っているか**を表しています。

$$
f'' < 0 \quad \Longleftrightarrow \quad f' \ \text{が減っている}
$$

いま $f'(a) = 0$ で、$f''(a) < 0$ だとします。$f''(a) < 0$ は「$a$ のところで $f'$ が減っている」という意味です。

$f'$ は $a$ で $0$ を通り、そこで減っているのですから、$a$ の**手前では正**、$a$ の**先では負**です。

$$
f' : \quad + \ \longrightarrow \ 0 \ \longrightarrow \ -
$$

これは [SL 5.6 第2節](../../ai-sl/05-calculus/sl-5-6.qmd#sign) の **maximum** の並びそのものです。だから maximum になります。

$f''(a) > 0$ なら、$f'$ は $a$ で増えているので
...
@tbl-ahl510-fails の $2$ つで確かめます。どちらも $x = 0$ で $f'(0) = 0$、$f''(0) = 0$ です。
...
**$0$ が出たので、この test では決まりません**（[第2節](#fails)）。$f'$ の符号に戻ります（[SL 5.6 第2節](../../ai-sl/05-calculus/sl-5-6.qmd#sign)）。
...
**正しくは**：$f'' > 0$ は **minimum**（谷）です（@eq-ahl510-test）。

$\cup$ の形が正、と結びつけてください（[第2節](#which-way)）。**この項目でいちばん多い失点です。**
:::
...
**正しくは**：$f'' = 0$ で分かるのは「この test が使えない」ことだけです。$f'$ の符号に戻ってください（[第2節](#fails)）。
:::
...
ダイアログに **`Derivative`** の欄があり、`1st Derivative` と `2nd Derivative` を選べます。`2nd Derivative` を選んで、もとの式と点を入れてください。

$f(x) = x^{3} - 3x^{2} - 9x + 5$ を $x = 3$ で $2$ 階微分すると $12$。自分で作った $f''(x) = 6x - 6$ に $x = 3$ を入れても $12$ ✓
...
@fig-ahl510-test の (a) が、この $3$ 本を重ねた絵です。
...
1. $f'' > 0$ を **minimum** と書いたか（@eq-ahl510-test）
2. 答えを**座標**で書いたか。$y$ は**もとの $f$** に代入したか
3. $f''(a) = 0$ が出たら、**$f'$ の符号に切り替えた**か（[第2節](#fails)）
...
**(b)** $f'' = 12x + 6$ に代入するだけです。**負が maximum、正が minimum** です（@eq-ahl510-test）。
...
**丸める前の $\ln 3$ を使って $y$ を計算してください**（[AHL 5.9c](ahl-5-9c.qmd#gdc-solve)）。
...
$x = 0$ で形が変わるので、そこが point of inflexion です（[第5節](#inflexion)）。
...
**「人数が減っている」と書くのは誤りです。** 減っているのは増加のペースであって、人数そのものではありません（[第5節](#inflexion)）。
...
::: {.callout-tip collapse="true"}
## 解説
**(b)** が、このページで押さえたいところです（[第2節](#fails)）。
...
::: {.callout-tip collapse="true"}
## 解説
点の数が微分の回数です（@tbl-ahl510-notation）。$\ddot{s}$ が acceleration です（@tbl-ahl510-kin）。
...
::: {.callout-tip collapse="true"}
## 解説
$f'' > 0$ は **minimum** です（@eq-ahl510-test）。この項目でいちばん多い取り違えです。

...
# AHL 5.15 — Slope fields（方向場） {#sec-ahl-5-15}

::: {.callout-note}
## What you should be able to do
- **slope field**（方向場、傾きを表す場）が何を表しているか説明できる。
...
[AHL 5.14](ahl-5-14.qmd) では微分方程式を変数分離で解きましたが、**手で解ける微分方程式はごく一部**です。$\dfrac{dy}{dx} = x - y$ も、AI HL の範囲では解けません。**それでも、解のグラフの形は分かります。** そのための道具が slope field です。
...
**`use and interpret`** ——**使うこと**（曲線をかき込むこと）と**読み取ること**の $2$ つで、**「解くこと」は求められていません。** 公式集の Topic 5 は **5.13 の次が 5.16** で、**5.14 と 5.15 の欄はありません。** 覚える公式もなく、やることは「代入して、線分をかく」だけです。ただし**やり方そのものは覚える必要があります。**
...
@fig-ahl515-build がその流れです。(a) で $1$ 点、(b) で数点、(c) で格子全体、(d) で**解曲線**（**solution curve**）です。
...
### 3. 線分をたどると、解曲線（solution curve）になります {#follow}
...
**このページで扱うのは、initial condition（初期条件）から解が $1$ つに定まる微分方程式です。** その前提のもとで
...
| どの**行**も、線分の向きが同じ | 傾きが $x$ だけで決まる |
| どの**列**も、線分の向きが同じ | 傾きが $y$ だけで決まる |
...
::: {.callout-tip}
## 「どの行も同じ」「どの列も同じ」の見分け
@fig-ahl515-features の (a) は $\dfrac{dy}{dx} = x$ です。右辺に $y$ がないので、**上下に動いても線分の向きが変わりません。**

(b) は $\dfrac{dy}{dx} = y$ です。右辺に $x$ がないので、**左右に動いても線分の向きが変わりません。**
...
理由はこうです。もし同じ点を $2$ 本の異なる解曲線が通ると、**その点を initial condition とする解が複数あることになります。** しかし、ここで扱う微分方程式では **initial condition から解が $1$ つに定まる**ので、そのような交差は起こりません（[第4節](#particular)）。
...
*For the differential equations considered here, an initial condition determines a unique solution. Therefore, two distinct solution curves cannot pass through the same point.*
:::

（ここで扱う微分方程式では、initial condition が解を $1$ つに定める。したがって、$2$ 本の異なる解曲線が同じ点を通ることはない、という意味です。）
:::
...
::: {.callout-note appearance="simple"}
数学としては、これは **existence and uniqueness theorem**（存在と一意性の定理）と呼ばれるもので、$f$ がなめらかなら成り立ちます。**このページで扱うのは、この一意性が成り立つ微分方程式だけ**です。

**IB では、この定理の名前も証明も要りません。** 「initial condition が解を $1$ つに定める」で十分です。
:::
...
::: {.model-answer}
*The line $y = -x - 1$ is itself a solution curve. To move from one side of it to the other, another solution curve would have to cross it, and at the crossing point two distinct solution curves would pass through the same point. For the differential equations considered here, an initial condition determines a unique solution, so that is impossible.*
:::
:::

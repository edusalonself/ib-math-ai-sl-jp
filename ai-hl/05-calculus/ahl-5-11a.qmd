...
**この $5$ つは、公式集の $5.11$ の欄にそのまま印刷されています。** 覚える必要はありません。身につけるべきなのは、**どの欄を見るか**です。$\dfrac{1}{x}$ だけは $5.5$ の欄ではなく $5.11$ の欄、というのがこの項目の分かれ道です（[第2節](#minus-one)）。
...
![Why the antiderivative of 1/x carries a modulus](img/ahl-5-11a-lnabs.svg){#fig-ahl511a-lnabs width=68%}

@fig-ahl511a-lnabs を見てください。赤の破線が $\dfrac{1}{x}$、青が $\ln|x|$ です。**$x < 0$ の側にも、ちゃんと青い曲線があります。** modulus を付けておけば、**$1$ つの式で両側をまかなえます。**
...
### 5. 定積分（definite integral） {#definite}

端の付いた積分を **定積分**（**definite integral**）、付いていない $\displaystyle\int f(x)\,dx$ を **不定積分**（**indefinite integral**）といいます。

**定積分には $+C$ が要りません。** **積分定数**（**constant of integration**）が引き算で消えるからです（[SL 5.5](../../ai-sl/05-calculus/sl-5-5.qmd#definite)）。

$$
\int_{a}^{b} f(x)\,dx = \bigl[F(x)\bigr]_{a}^{b} = F(b) - F(a)
$$

$a$ を**下端**（**lower limit**）、$b$ を**上端**（**upper limit**）、$f(x)$ を**被積分関数**（**integrand**）、$F(x)$ を**原始関数**（**antiderivative**）といいます。**試験の問題文は英語なので、右側の語で覚えてください。**

定積分では、**グラフと $x$ 軸ではさまれた部分の面積**を求めることができます。くわしくは [AHL 5.12a](ahl-5-12a.qmd) で習いますが、ここでも $1$ つ見ておきます。

![A definite integral as the area under the curve](img/ahl-5-11a-area.svg){#fig-ahl511a-area width=68%}

@fig-ahl511a-area は、$y = \dfrac{1}{x}$ と $x$ 軸ではさまれた $x = 1$ から $x = 3$ までの面積です。
...
画面に積分のテンプレートが出るので、**見たままの形**で入れます。分数もテンプレートのまま入ります。
...
HL の試験では、$^{\circ}$ が書いていなければ radian です。**$\displaystyle\int_{0}^{\frac{\pi}{4}}\frac{1}{\cos^{2}x}\,dx$ のように、積分の端に $\pi$ が出てきたら、まず設定を確かめてください。**
...
@fig-ahl511a-lnabs の青い曲線が、左右で対称になっているのはこのためです。
...
- $3\sin x$ → $-3\cos x$（@tbl-ahl511a-pairs。**マイナスが付きます**）
- $-2e^{x}$ → $-2e^{x}$（@tbl-ahl511a-pairs。$e^{x}$ は変わりません）
...
**検算。** 電卓の `Integral` で $\displaystyle\int_{1}^{4}\left(\sqrt{x} + \frac{3}{x^{2}}\right)dx$ を計算すると $6.9167$。$\dfrac{83}{12} = 6.9167$ ✓（[GDC 第2節](#gdc-integral)）
...
**(b)** [Hence show that $\displaystyle\int_{0}^{\frac{\pi}{4}} \frac{1}{\cos^{2}x}\,dx = 1$.]{.q-en}
...
**(a)** 公式集の $5.11$ の欄にあります（@tbl-ahl511a-pairs）。
...
**検算。** 電卓を Radian にして `Integral` で計算すると $1$ です ✓（[第7節](#radian)）
...
**(b)**
$$
\int_{0}^{\frac{\pi}{4}} \frac{1}{\cos^{2}x}\,dx = \bigl[\tan x\bigr]_{0}^{\frac{\pi}{4}} = \tan\frac{\pi}{4} - \tan 0 = 1 - 0 = 1
$$
:::
:::
...
積分のテンプレートが出るので、**画面に出てくる形のまま**入れます。@exm-ahl511a-tank (d) なら、こうです。
...
::: {.callout-warning}
## 電卓は「式」を返しません
`Integral` が返すのは、**ある区間の値だけ**です。$\displaystyle\int f(x)\,dx$（不定積分）を聞かれたら、**原始関数の式を自分で書く**しかありません（[第5節](#by-gdc)）。
:::
...
下端と上端を指定すると、**塗られた面積と値**が画面に出ます（[SL 5.5](../../ai-sl/05-calculus/sl-5-5.qmd#graphs)）。@fig-ahl511a-area と同じ絵になります。
...
::: {.callout-tip collapse="true"}
## 解説
項ごとに積分します（[第4節](#terms)）。$\cos$ の積分にはマイナスが付きません（@tbl-ahl511a-pairs）。
...
[6]{.ex-no} [Find the exact value of $\displaystyle\int_{0}^{\frac{\pi}{3}} \sin x\,dx$.]{.q-en}
...
## 解答例（答案用紙に書くこと）
$$
\int_{0}^{\frac{\pi}{3}} \sin x\,dx = \bigl[-\cos x\bigr]_{0}^{\frac{\pi}{3}}
$$
$$
...
::: {.callout-tip collapse="true"}
## 解説
$\sin$ の積分は $-\cos$ です（@tbl-ahl511a-pairs）。
...
$\dfrac{\pi}{3}$ は $60^{\circ}$ で、$\cos 60^{\circ} = \dfrac{1}{2}$ です（[AHL 5.9a 第4節](ahl-5-9a.qmd#radian)）。
...
[7]{.ex-no} [Find the exact value of $\displaystyle\int_{\frac{\pi}{6}}^{\frac{\pi}{3}} \frac{1}{\cos^{2}x}\,dx$.]{.q-en}
...
## 解答例（答案用紙に書くこと）
$$
\int_{\frac{\pi}{6}}^{\frac{\pi}{3}} \frac{1}{\cos^{2}x}\,dx = \bigl[\tan x\bigr]_{\frac{\pi}{6}}^{\frac{\pi}{3}}
$$
$$
...
::: {.callout-tip collapse="true"}
## 解説
$\dfrac{1}{\cos^{2}x}$ の積分は $\tan x$ です（@tbl-ahl511a-pairs）。
...
::: {.callout-tip collapse="true"}
## 解説
どちらも公式集の $5.11$ の欄にあります（@tbl-ahl511a-pairs）。

...
- $\sin x$、$\cos x$、$\tan x$、$e^{x}$、$\ln x$ の **derivative**（導関数）を書ける。
- $x^{n}$ の公式 $f(x) = ax^{n} \Rightarrow f'(x) = anx^{\,n-1}$ を、**$n$ が分数のとき**にも使える。
- $\sqrt{x}$ や $\dfrac{4}{\sqrt{x}}$ を **$x^{n}$ の形に書き直してから**微分できる。
...
### 1. SL では、$x^{n}$ の形しか微分できませんでした {#idea}
...
$3x^{5}$ も $\dfrac{6}{x^{2}} = 6x^{-2}$ も、これで微分できます。**けれども $\sin x$ や $e^{x}$ は、$x^{n}$ の形をしていません。** ですから、この公式では手が出ませんでした。

この **5.9a** で増えるのは、次の $2$ つです。

1. **$n$ が分数のときも使える**ようになる（$\sqrt{x}$ が微分できる）
2. **$x^{n}$ 以外の $5$ つ**の derivative（導関数）が加わる（$\sin$、$\cos$、$\tan$、$e^{x}$、$\ln x$）

シラバスの 5.9 は、これで終わりではありません。**$2$ つの式を組み合わせた形の微分**と、**同時に変わる $2$ つの量**の扱いが [AHL 5.9b](ahl-5-9b.qmd) にあります。このページは、その土台になります。

### 2. $n$ が分数でもよくなります {#rational}

HL では、$x^{n}$ の $n$ に **rational number**（有理数、つまり分数で書ける数）を入れてよいことになります。SL では整数だけでした。
...
**この $5$ つは、公式集の $5.9$ の欄にそのまま印刷されています。**試験中に配られるので、覚える必要はありません。覚えるべきなのは、**どれを使う場面なのか**と、**電卓を radian にしておくこと**（[第4節](#radian)）の $2$ つです。
...
#### $\cos^{2} x$ では、$2$ 乗は $\cos x$ 全体に付きます {#tan}
...
$\dfrac{d}{dx}\sin x = \cos x$（@tbl-ahl59a-five）は、**$x$ を radian で測っているときにだけ**成り立ちます。

degree で測ると $\dfrac{\pi}{180}$ という余分な数が付いてしまいます（[Why it works](#why-radian)）。
...
derivative（導関数）は「$x$ が少し増えたとき、$y$ がどれだけ増えるか」の割合です（[SL 5.1](../../ai-sl/05-calculus/sl-5-1.qmd#chord)）。ですから、**$x$ を何で測るかを変えると、値が変わります。**
...
つまり、**微分しても変わらない**ということです。$y = e^{x}$ のグラフでは、$x = 1$ における高さと傾きがどちらも $e$ になります。
...
**(a)** 項ごとに微分します（[第5節](#terms)）。使うのは @tbl-ahl59a-five の $3$ つです。
...
**(a)** 根号を $x^{n}$ の形に直します（@tbl-ahl59a-rewrite）。
...
**(b)** $x^{n}$ の公式を、それぞれの項に使います（@eq-ahl59a-power）。$n$ が分数でも、やることは同じです（[第2節](#rational)）。
...
**(a)** $\tan x$ の導関数は $\dfrac{1}{\cos^{2}x}$ です（@tbl-ahl59a-five）。$2$ 乗するのは $\cos x$ 全体です（[第3節](#tan)）。
...
**(a)** 定数 $4$ の微分は $0$、$2\sin t$ の微分は $2\cos t$ です（@tbl-ahl59a-five）。
...
**正しくは**：$-\sin x$ です（@tbl-ahl59a-five）。$\sin \to \cos$ は符号なし、$\cos \to -\sin$ は符号ありです（[第3節](#five)）。
...
$x^{n}$ は $x$ が**下**、$e^{x}$ は $x$ が**上**です。別の形なので、別の公式です。

**正しくは**：$e^{x}$ のままです（@tbl-ahl59a-five）。
:::
...
微分のテンプレートが出るので、**画面に出てくる形のまま**入れます。

$$
\left. \frac{d}{dx}\left(3\sin x - 2e^{x} + \ln x\right) \right|_{x=1}
$$
...
- $f1(x) = 3\sin(x) - 2e^{x} + \ln(x)$
- $f2(x) = 3\cos(x) - 2e^{x} + \dfrac{1}{x}$

**$f_1$ が水平なところで、$f_2$ が $x$ 軸を横切っていれば合っています。**（[SL 5.6](../../ai-sl/05-calculus/sl-5-6.qmd#fdash-gdc)）

たとえば $f_1 = \sin x$、$f_2 = \cos x$ なら、$\sin x$ が水平になる $x = \dfrac{\pi}{2}$ で、$\cos x$ が $x$ 軸を横切っています。
...
**$2$ つ目でマイナスが $2$ 回出ます。** $\cos$ の微分のマイナスと、もとの式のマイナスです。$(-3) \times (-1) = +3$ なので、答えは $+3\sin x$ になります（@tbl-ahl59a-five）。
...
::: {.callout-tip collapse="true"}
## 解説
$e^{x}$ の微分は $e^{x}$、$\ln x$ の微分は $\dfrac{1}{x}$ です（@tbl-ahl59a-five）。
...
::: {.callout-tip collapse="true"}
## 解説
まず **$x^{n}$ の形に書き直します**（@tbl-ahl59a-rewrite）。$\dfrac{8}{\sqrt{x}} = 8x^{-\frac{1}{2}}$ です。
...
::: {.callout-tip collapse="true"}
## 解説
$\dfrac{\pi}{3}$ は $60^{\circ}$ です。$\cos 60^{\circ} = \dfrac{1}{2}$ ですから、$2$ 乗して $\dfrac{1}{4}$、逆数をとって $4$ です。
...
::: {.callout-tip collapse="true"}
## 解説
$2$ つの項を、それぞれ $x^{n}$ の形に直します（@tbl-ahl59a-rewrite）。
...
::: {.callout-tip collapse="true"}
## 解説
**(a)** $x$ の微分は $1$、$\ln x$ の微分は $\dfrac{1}{x}$ です（@tbl-ahl59a-five）。
...
::: {.callout-tip collapse="true"}
## 解説
$\cos x$ を微分するとマイナスが付きます（@tbl-ahl59a-five）。$\sin x$ を微分するときは付きません。**片方だけに付く**ので、取り違えやすいところです。
...
$$
\left. \frac{d}{dx}\left(\ln x + \sin x\right) \right|_{x=2}
$$

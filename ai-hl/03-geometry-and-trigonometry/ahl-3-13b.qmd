...
**(d)** (a) の答えを、自分の大きさで割ります（[AHL 3.10b 第6節](ahl-3-10b.qmd#unit)）。
...
$\begin{pmatrix} -6 \\ 8 \\ 10 \end{pmatrix}$ が返ります。`crossP(` は **`menu → 7: Matrix & Vector → C: Vector`** の中で、`dotP(` と同じところです（[AHL 3.13a の GDC 第1節](ahl-3-13a.qmd#gdc-dotp)）。

ベクトルは **`[2;-1;2]`**（角かっこ、区切りはセミコロン）で入れます。
...
まず、**同じ頂点 $A$ から出る $2$ 辺**を作ります（[第8節](#area)）。$\overrightarrow{AB} = \mathbf{b} - \mathbf{a}$ です（[AHL 3.10b 第4節](ahl-3-10b.qmd#ab)）。

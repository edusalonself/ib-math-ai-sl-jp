...
**@eq-ahl310b-mag は試験中に見られます。暗記は問われません。** ただ、$2$ 次元のときにどう書くのか（[第1節](#magnitude)）、なぜこの形なのか（[Why it works](#why-it-works)）は、自分で言えるようにしておいてください。
...
**resultant を「足す」ところは [AHL 3.10a](ahl-3-10a.qmd#add) で終わりました。その大きさと向きを出すのが、このページ**です（[第8節](#resultant)）。
:::

::: {.callout-warning}
## `normal` という語が、$2$ か所で別の意味に使われています
...
**$\dfrac{\mathbf{v}}{\lvert \mathbf{v} \rvert}$ は $\mathbf{v}$ と同じ向き**で、$\mathbf{v}$ に垂直ではありません（[第6節](#unit)）。**同じ英語が別の意味で出てくる**ので、そこだけ気を付けてください。
:::
...
### 2. $3$ 次元では、三平方の定理を $2$ 回使います {#three-d-mag}

@fig-ahl310b-magnitude の (b) は、$\mathbf{v} = \begin{pmatrix} 2 \\ 3 \\ 6 \end{pmatrix}$ を直方体の対角線として描いたものです。

**手順は $2$ 段**です。

1. **床の対角線**を出す … $\sqrt{2^2+3^2}$
2. **床の対角線と高さ**で、もう $1$ 度 … $\sqrt{\left(\sqrt{2^2+3^2}\right)^2 + 6^2}$

$2$ 段目で $\sqrt{\ }$ が $2$ 乗されて消えるので、結局

$$
\lvert \mathbf{v} \rvert = \sqrt{2^2+3^2+6^2} = \sqrt{49} = 7
$$

**途中の $\sqrt{13}$ は、計算しません。** ここが大事なところで、だからこそ公式集の @eq-ahl310b-mag は $3$ つの $2$ 乗を並べた形になっています（[Why it works](#why-it-works)）。

**$2$ 次元は、$v_3 = 0$ の場合**と思ってかまいません。@eq-ahl310b-mag に $v_3 = 0$ を入れると @eq-ahl310b-mag2 になります。

### 3. position vector — 点をベクトルで表す {#position}
...
### 4. $\overrightarrow{AB} = \mathbf{b}-\mathbf{a}$ — 行き先ひく出発点 {#ab}
...
**逆にすると、向きが正反対になります。** 長さは同じです（[第5節](#distance)）。
:::

### 5. $2$ 点間の距離は $\lvert \overrightarrow{AB} \rvert$ です {#distance}
...
**長さだけが $1$ になります。** たしかめると $\sqrt{0.6^2+0.8^2} = \sqrt{1} = 1$ です。
...
### 7. 「速さ $7$ で、$3\mathbf{i}+4\mathbf{j}$ の向き」 {#direction}
...
@fig-ahl310b-unit の (b) が、その $3$ 段です。答えは
...
**電卓の角度設定も合わせます**（[GDC 第5節](#gdc-angle)）。設定が radian のままだと、$\tan^{-1}$ は radian で返ってきます。
:::

### 9. rescale — 長さを指定して作る {#rescale}
...
**$\lvert k \rvert$ です。$k$ ではありません。** $k$ が負のとき、$k\lvert \mathbf{v} \rvert$ は負の数になってしまい、長さになりません（[Why it works](#why-it-works)）。

**$3$ 次元でも、まったく同じ**です。成分が $1$ つ増えるだけで、割り算も掛け算も変わりません。

## Why it works

**なぜ $\lvert \mathbf{v} \rvert = \sqrt{v_1^{\,2}+v_2^{\,2}}$ なのか。**

$\mathbf{v} = \begin{pmatrix} v_1 \\ v_2 \end{pmatrix}$ の矢印は、**横に $v_1$、たてに $v_2$ 進んだ**ものでした。この $2$ 本は直角に交わるので、直角三角形ができます。斜辺が矢印そのものです。

三平方の定理は「$\text{斜辺}^2 = \text{たて}^2 + \text{よこ}^2$」ですが、**辺の長さは $\lvert v_1 \rvert$ と $\lvert v_2 \rvert$** です。ところが

$$
\lvert v_1 \rvert^2 = v_1^{\,2}
$$

なので、絶対値を書く必要がありません。**$2$ 乗が符号を消してくれる**ので、@eq-ahl310b-mag2 は成分をそのまま入れる形で書けます。

**$3$ 次元では、これを $2$ 回使います。** まず床の上で $\sqrt{v_1^{\,2}+v_2^{\,2}}$、次にそれを $1$ 辺、高さ $v_3$ をもう $1$ 辺とする直角三角形で

$$
\lvert \mathbf{v} \rvert = \sqrt{\left(\sqrt{v_1^{\,2}+v_2^{\,2}}\right)^{2} + v_3^{\,2}} = \sqrt{v_1^{\,2}+v_2^{\,2}+v_3^{\,2}}
$$

$\sqrt{\ }$ が $2$ 乗で消えるので、**中身がそのまま足し合わさります。** これが @eq-ahl310b-mag の形の理由です。

**なぜ $\lvert k\mathbf{v} \rvert = \lvert k \rvert \lvert \mathbf{v} \rvert$ なのか。**

$$
\lvert k\mathbf{v} \rvert = \sqrt{(kv_1)^2+(kv_2)^2} = \sqrt{k^2\left(v_1^{\,2}+v_2^{\,2}\right)} = \sqrt{k^2}\,\sqrt{v_1^{\,2}+v_2^{\,2}}
$$

ここで $\sqrt{k^2} = \lvert k \rvert$ です。**$k$ ではありません。** $k = -2$ なら $\sqrt{4} = 2$ で、$-2$ ではないからです。

**この式から、単位ベクトルの理由がすぐ出ます。** $\mathbf{v} \neq \mathbf{0}$ のとき $k = \dfrac{1}{\lvert \mathbf{v} \rvert}$ とおくと

$$
\left\lvert \frac{\mathbf{v}}{\lvert \mathbf{v} \rvert} \right\rvert = \frac{1}{\lvert \mathbf{v} \rvert}\,\lvert \mathbf{v} \rvert = 1
$$

$\lvert \mathbf{v} \rvert > 0$ なので $k$ は正で、**向きも変わりません。**

**なぜ $\overrightarrow{AB} = \mathbf{b}-\mathbf{a}$ なのか。**

nose to tail でつなぐと $\overrightarrow{OA} + \overrightarrow{AB} = \overrightarrow{OB}$ です（[AHL 3.10a 第5節](ahl-3-10a.qmd#add)）。両辺から $\overrightarrow{OA}$ を引けば

$$
\overrightarrow{AB} = \overrightarrow{OB} - \overrightarrow{OA} = \mathbf{b} - \mathbf{a}
$$

**足し算の式を移項しただけ**です。新しい規則ではありません。
...
**(b)** 行き先ひく出発点です（@eq-ahl310b-ab）。**$4$ を引くので、$2$ つ目は $-8$** です。
...
**距離を聞かれているだけなら、逆でも答えは同じ**です（[第5節](#distance)）。しかし**ベクトルを聞かれていたら、符号がすべて逆になって不正解**です。
:::
...
### 2. 成分が多いときは、`dotP(` が使えます {#gdc-dotp}

$\lvert \mathbf{v} \rvert = \sqrt{\mathbf{v} \cdot \mathbf{v}}$ なので、次のようにも書けます。

```
√(dotP([2;-3;6],[2;-3;6]))
```

$7$ が返ります。`dotP(` は `menu → 7: Matrix & Vector → C: Vector` の中にあります（同じところに `crossP(` もあります。どちらも [AHL 3.13a](ahl-3-13a.qmd)、[AHL 3.13b](ahl-3-13b.qmd) で使います）。

**成分の打ち間違いが $1$ か所で済む**のが、この書き方の利点です。

::: {.callout-warning}
## 大きさを $1$ 発で出すコマンドは、期待しないでください
この本が想定している**非 CAS** 機のリファレンスには、`unitV(` や `norm(` は載っていません。

**$\sqrt{\ }$ と `dotP(` で足ります。** 単位ベクトルも、$\lvert \mathbf{v} \rvert$ を出してから割ります（[GDC 第4節](#gdc-unit)）。
:::

### 3. 点を入れておくと、$\overrightarrow{AB}$ が楽です {#gdc-store}
...
### 5. 向きの角度は、角度設定を確かめてから {#gdc-angle}
...
**AHL Topic 3 の既定は radian** ですが、**方位は度で答えます**（[第8節](#resultant)）。$\tan^{-1}$ を使う前に、設定を見てください。
...
$3$ 次元でも、成分が $1$ つ増えるだけです（@eq-ahl310b-mag、[第2節](#three-d-mag)）。
...
行き先ひく出発点です（@eq-ahl310b-ab）。**$2$ つ目は $2-(-1) = 3$** で、ここが引っかかりどころです。
...
まず長さ、次に割る、の $2$ 段です（@eq-ahl310b-unit）。**符号は、そのまま残ります。** $\dfrac{1}{10}$ は正なので、向きは変わりません。
...
@tbl-ahl310b-steps の $3$ 段です。$\lvert -3\mathbf{i}+4\mathbf{j} \rvert = 5$、単位ベクトルは $-0.6\mathbf{i}+0.8\mathbf{j}$、それに $20$ を掛けます。

**$20(-3\mathbf{i}+4\mathbf{j}) = -60\mathbf{i}+80\mathbf{j}$ としないでください。** これは長さ $100$ です（[第7節](#direction)）。
...
$3$ 次元でも手順は同じです（[第9節](#rescale)）。**$3$ つとも $7$ で割ります。**

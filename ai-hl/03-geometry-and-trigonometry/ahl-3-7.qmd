...
- **radian**（ラジアン）が「**半径ぶんの長さの弧**に対する角」であることを説明できる。
...
### 1. radian は「半径ぶんの弧」に対する角です {#idea}
...
@fig-ahl37-radian の (a) を見てください。半径 $r$ の円で、**長さがちょうど $r$ の弧**を取ります。その弧を見込む角が **$1$ radian** です。

$$
1 \text{ radian} = \text{（弧の長さが半径と等しくなる角）}
$$
...
シラバスの AHL Topic 3 の前置きが、はっきり書いています。
...
### 8. 逆向きにも使えます {#backwards}

$l$ や $A$ が与えられて、$\theta$ や $r$ を求める問題もよく出ます。**式を変形するだけ**です。

| 分かっているもの | 求めるもの | 使う形 |
|:--|:--|:--|
| $l$ と $r$ | $\theta$ | $\theta = \dfrac{l}{r}$ |
| $l$ と $\theta$ | $r$ | $r = \dfrac{l}{\theta}$ |
| $A$ と $r$ | $\theta$ | $\theta = \dfrac{2A}{r^{2}}$ |
| $A$ と $\theta$ | $r$ | $r = \sqrt{\dfrac{2A}{\theta}}$ |

: 逆向きの使い方 {#tbl-ahl37-back}

$\theta = \dfrac{l}{r}$ という形は、**radian の定義そのもの**です。$l = r$ なら $\theta = 1$——[第1節](#idea)で言ったとおりです。

**出てきた $\theta$ はラジアン**です。度で答えよと言われていたら、$\dfrac{180}{\pi}$ をかけてください。

扇形の**周**（perimeter）を聞かれることもあります。弧だけではなく、**$2$ 本の半径も入ります。**

$$
\text{perimeter} = l + 2r = r\theta + 2r
$$ {#eq-ahl37-perim}

[SL 3.4](../../ai-sl/03-geometry-and-trigonometry/sl-3-4.qmd#perimeter) と同じ考え方です。
...
**(a)** @eq-ahl37-arc を $\theta$ について解きます（@tbl-ahl37-back）。

$$
\theta = \frac{l}{r} = \frac{15}{6} = 2.5 \text{ radians}
$$

**この形が、radian の定義そのもの**です（[第8節](#backwards)）。弧が半径の $2.5$ 倍あるので、角は $2.5$ radians です。
...
::: {.callout-warning}
## 扇形の周に、半径を入れ忘れる
`perimeter` を聞かれて、弧の長さだけを答える誤りです。

$$
\text{perimeter} = r\theta + 2r
$$

**扇形はピザの $1$ 切れの形**です。まわりを $1$ 周すると、丸い部分と、まっすぐな辺が $2$ 本あります（@eq-ahl37-perim）。
:::
...
TI-Nspire CX II は **Numeric（非 CAS）**です。$\dfrac{4\pi}{5}$ のような**記号のままの答えは返ってきません。** 小数の $2.51327\ldots$ が出ます。
...
度の記号は**記号パレット**から選びます（TI 公式のガイドでは、handheld で `ctrl` と `k` を押すとパレットが開く、と書かれています）。設定を切りかえるより、こちらのほうが安全です（[AHL 5.9a](../05-calculus/ahl-5-9a.qmd) と同じやり方です）。
...
@eq-ahl37-arc を $r$ について解きます（@tbl-ahl37-back）。

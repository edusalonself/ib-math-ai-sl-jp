...
![What $\cos\theta$ and $\sin\theta$ mean, and the signs in each quadrant](img/ahl-3-8-unit-circle.svg){#fig-ahl38-circle width=100%}

@fig-ahl38-circle の (a) を見てください。**$\cos\theta$ は横の座標、$\sin\theta$ は縦の座標**です。それ以上の意味はありません。
...
**第 $1$ 象限の外では、この対応は使えません。** @fig-ahl38-circle の (a) の $\theta$ は第 $2$ 象限なので、垂線で作った三角形の底辺の**長さ**は $0.589$ ですが、$\cos\theta$ は $-0.589$ です。**長さは正、座標は符号つき**——ここが違います。
...
@fig-ahl38-circle の (b) が、@tbl-ahl38-signs を絵にしたものです。
...
**つまり、$\sin\dfrac{\pi}{3} = \dfrac{\sqrt{3}}{2}$ のような値を暗記する必要はありません。** 試験では電卓が使えます。
...
: ambiguous case の手順 {#tbl-ahl38-steps}

::: {.callout-important}
## $2$ つ出るとはかぎりません
@tbl-ahl38-steps の $4$ 段目は、**落とすためにあります。**
...
**三角関数はくり返すので、解があるときは無限にあります。** だから問題文は必ず、$0 \leq x \leq 2\pi$ のような**区間を指定します。**（$\sin x = 2$ のように、$1$ つも解がない式もあります。） その区間の中の解を、**全部**出すのが仕事です。
...
$\sin B < 1$ なので、候補が $2$ つあります（@tbl-ahl38-steps）。
...
**$A + B_2 < 180^{\circ}$ を確かめてください**（@tbl-ahl38-steps）。通らなければ、その三角形は存在しません。
...
度の記号は**記号パレット**から選びます（TI 公式のガイドでは、handheld で `ctrl` と `k` を押すとパレットが開く、と書かれています）。**設定を切りかえるより安全です**（[AHL 3.7](ahl-3-7.qmd#gdc-degree) と同じやり方です）。
...
**電卓は「もう $1$ つある」とは教えてくれません**（[第3節](#quadrants)）。@tbl-ahl38-steps の手順を、自分で回してください。
...
::: {.callout-warning}
## $\cos^{2}\theta+\sin^{2}\theta$ は、答えの検算になりません
`cos(x)^2+sin(x)^2` は、**$x$ がどんな値でも $1$ を返します。** 角の設定が違っていても $1$ です。

ですから、これで確かめられるのは「**$2$ か所に同じ角を打てたか**」だけです。$\cos^{2}\theta$ を $\cos(\theta^{2})$ と打ってしまった、という取り違えは見つかります（[Common errors](#common-errors) の $4$ つ目）。

**答えそのものが正しいかは、$1$ つ目の「もとの式に戻す」で確かめてください。**
:::
...
@tbl-ahl38-steps の手順どおりです。$\sin B = 0.7194 < 1$ なので、候補が $2$ つ出ます。

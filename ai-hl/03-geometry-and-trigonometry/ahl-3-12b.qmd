...
このページで使う道具は、**すでに書いてある式ばかり**です。

- 微分と積分 … [AHL 5.9b](../05-calculus/ahl-5-9b.qmd)、[AHL 5.11a](../05-calculus/ahl-5-11a.qmd)、[AHL 5.13](../05-calculus/ahl-5-13.qmd)
- 大きさ $\lvert \mathbf{v} \rvert$ … [AHL 3.10b](ahl-3-10b.qmd#magnitude)（公式集にあります）

**新しく覚える式はありません。** 変わるのは、**それを成分ごとに使う**というところだけです（[Why it works](#why-it-works)）。
:::
...
**この Example は、そのままこのページの主役にします**（[第1節](#idea)）。
...
微分も積分も、**それぞれの行に、別々に**します（[Why it works](#why-it-works)）。**新しい微分法や積分法は要りません。**
:::
...
![Projectile motion and circular motion](img/ahl-3-12b-special.svg){#fig-ahl312b-special width=100%}

**projectile motion**（放物運動）は、**加速度が下向きの定数**である運動です。
...
@fig-ahl312b-special の (a) が、その例です。$\mathbf{v} = \begin{pmatrix} 15 \\ 20-10t \end{pmatrix}$ で、$\mathbf{a} = \begin{pmatrix} 0 \\ -10 \end{pmatrix}$ です。@eq-ahl312b-projectile で $v_{x0} = 15$、$v_{y0} = 20$、$g = 10$ とした形です。
...
@fig-ahl312b-special の (b) が、$R = 5$、$\omega = 2$ の場合です。速さは $10$ で一定ですが、**矢印はまわり続けています。**
...
Guidance の `Link to: phase shift (AHL 1.13)` は、ここのことです。[AHL 1.13b 第7節](../01-number-and-algebra/ahl-1-13b.qmd#interpret) では $A\cos(\omega t + B)$ の形で、時間軸の上での移動量が $-\dfrac{B}{\omega}$ 秒でした。**$f(t-a)$ の $a$ にあたるのが、その $-\dfrac{B}{\omega}$ です。**
:::
...
**横の変化と縦の変化が互いに影響しない**からこうなります。これは [AHL 3.10a の Why it works](ahl-3-10a.qmd#why-it-works) で、足し算を成分ごとにしてよい理由として書いたことと、同じ話です。
...
`menu → 4: Calculus` の中の **`Numerical Derivative at a Point`**（微分のテンプレートが出ます）と **`Numerical Integral`**（`nInt(` が出ます）を使います。`nInt(` は、入力行に直に打っても構いません。

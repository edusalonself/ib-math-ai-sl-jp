...
このページでやるのは、**その graph を「計算できる表」に書き直すこと**です。図を見て数える代わりに、行列のかけ算に任せます。
...
長さ $0$ も数えるときだけ、単位行列を足します。`identity(` は `menu → Matrix & Vector → Create → Identity` から入れられます。

$\texttt{identity}\left(4\right) + a + a^{2} + a^{3}$

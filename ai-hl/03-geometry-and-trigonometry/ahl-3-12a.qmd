...
[AHL 3.11](ahl-3-11.qmd) の $\mathbf{r} = \mathbf{a} + \lambda\mathbf{b}$ で、**$\mathbf{a}$ を $\mathbf{r}_0$、$\lambda$ を $t$、$\mathbf{b}$ を $\mathbf{v}$ と書き直しただけ**です（@eq-ahl312a-correspond、@tbl-ahl312a-correspond）。
...
**speed**（速さ）は、velocity の**大きさ**です（[AHL 3.10b 第4節](ahl-3-10b.qmd#distance)）。
...
**$2$ 直線で別の文字を使うのは、[AHL 3.11 第9節](ahl-3-11.qmd#intersect) と同じ理由**です。しかもこのページでは、その $2$ つの値には**時刻**という意味があります。
...
**「ゴールひくスタート」**でした（[AHL 3.10b 第3節](ahl-3-10b.qmd#ab)）。$A$ から見て $B$ がどこにいるか、を表します。
...
: 最接近を出す $5$ つの手順 {#tbl-ahl312a-steps}
...
**(a)** 経路は直線なので、[AHL 3.11 第9節](ahl-3-11.qmd#intersect) のやり方です。**文字は $2$ 種類**にします。
...
**Matrix** として入れて、そのまま計算します（[AHL 3.10a の GDC 第1節](ahl-3-10a.qmd#gdc-enter)）。
...
`menu → Algebra` の中の **Solve System of Linear Equations** を使います（[AHL 3.11 の GDC 第3節](ahl-3-11.qmd#gdc-linsolve)）。
...
**(a)** 経路の交点なので、文字は $2$ 種類です（[AHL 3.11 第9節](ahl-3-11.qmd#intersect)）。$2$ 本目から $2\lambda = 3\mu$、$1$ 本目に入れて $1+3\mu = 9-\mu$、つまり $\mu = 2$、$\lambda = 3$ です。

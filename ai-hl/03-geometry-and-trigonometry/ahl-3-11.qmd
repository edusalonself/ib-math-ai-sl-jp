...
![One point on the line, and one direction](img/ahl-3-11-idea.svg){#fig-ahl311-idea width=100%}

@fig-ahl311-idea がその図です。式にすると

$$
\mathbf{r} = \mathbf{a} + \lambda\mathbf{b}
$$

- $\mathbf{r}$ … 直線上の**点の位置ベクトル**（[AHL 3.10b 第2節](ahl-3-10b.qmd#position)）
...
@tbl-ahl311-lambda の点は、@fig-ahl311-idea に打ってあります。**$\lambda$ が $1$ 増えるごとに、$\mathbf{b}$ だけ動きます。**
...
![The equation of a line is not unique](img/ahl-3-11-notunique.svg){#fig-ahl311-notunique width=100%}

@fig-ahl311-notunique を見てください。@fig-ahl311-idea と**同じ直線**を、別の式で書いたものです。
...
[AHL 3.10b 第6節](ahl-3-10b.qmd#direction) では、$3\mathbf{i}+4\mathbf{j}$ の長さが $5$ であることが問題になりました。
...
- **$\mathbf{b}$** … $\overrightarrow{AB} = \mathbf{b}-\mathbf{a}$（[AHL 3.10b 第3節](ahl-3-10b.qmd#ab)）
...
**(a)** ゴールひくスタートです（[AHL 3.10b 第3節](ahl-3-10b.qmd#ab)）。
...
**単位ベクトルが要るのは [AHL 3.10b 第6節](ahl-3-10b.qmd#direction) の「速さ $\times$ 向き」の場面**で、直線の式では要りません。
:::
...
$\mathbf{a}$ と $\mathbf{b}$ を **Matrix** として入れて、そのまま計算します（[AHL 3.10a の GDC 第1節](ahl-3-10a.qmd#gdc-enter)）。

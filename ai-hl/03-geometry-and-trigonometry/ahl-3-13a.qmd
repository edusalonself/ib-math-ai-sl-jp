...
**scalar product**（スカラー積）は、$2$ つのベクトルから **$1$ つの数**を作る計算です。**成分どうしをかけて、足す**だけです。
...
この $\mathbf{v} \cdot \mathbf{w}$ を **dot product**（ドット積）とも言います。**scalar product と同じもの**です。$\cdot$ を落とさないでください。$\mathbf{v}\mathbf{w}$ という書き方はしません。
...
**大きさと、あいだの角から内積を出す形**です。公式集の $2$ 行目にあります。

$$
\mathbf{v} \cdot \mathbf{w} = \lvert \mathbf{v} \rvert \lvert \mathbf{w} \rvert \cos\theta
$$ {#eq-ahl313a-geom}

![The angle between two vectors](img/ahl-3-13a-idea.svg){#fig-ahl313a-idea width=62%}

ここで $\theta$ は、**$\mathbf{v}$ と $\mathbf{w}$ を同じ点から描いたときの、あいだの角**です（@fig-ahl313a-idea）。
...
**$2$ つのベクトルのあいだの角を出すための式**です。@eq-ahl313a-geom を $\cos\theta$ について解いたもので、公式集の $3$ 行目にあります。
...
![Reading the sign of the scalar product](img/ahl-3-13a-sign.svg){#fig-ahl313a-sign width=66%}

@fig-ahl313a-sign が、その $3$ つです。
...
$\mathbf{a} \cdot \mathbf{b}$ そのものではなく、$\lvert \mathbf{b} \rvert$ で割ることに注意してください。$\mathbf{b}$ を単位ベクトル（[AHL 3.10b 第5節](ahl-3-10b.qmd#unit)）に直してから内積を取っている、と見ることもできます。
...
$\hat{\mathbf{b}}$（「$\mathbf{b}$ ハット」と読みます）は、**$\mathbf{b}$ と同じ向きで長さを $1$ にそろえたベクトル**、つまり unit vector です。長さが $1$ なので、$\mathbf{a} \cdot \hat{\mathbf{b}}$ は **$\mathbf{a}$ をその向きにどれだけ進んだか**という数だけを取り出します。
...
$2$ つ目は、$\theta = 0$ を @eq-ahl313a-geom に入れれば出ます。**$\lvert \mathbf{v} \rvert = \sqrt{\mathbf{v} \cdot \mathbf{v}}$** と書けるのは、この事実によるものです（[AHL 3.10b 第1節](ahl-3-10b.qmd#magnitude)）。
...
**$-1$ から $1$ の外の値**が出たら、そこまでの計算が間違っています。
:::
...
$-14$ が返ります。`dotP(` は **`menu → Matrix & Vector → Vector`** の中です。同じところに `crossP(` もあり、そちらは [AHL 3.13b](ahl-3-13b.qmd) で使います。

ベクトルは **Matrix** として入れます（[AHL 3.10a の GDC 第1節](ahl-3-10a.qmd#gdc-enter)）。`Number of rows` を $2$、`Number of columns` を $1$ にすると、$\begin{bmatrix} 3 \\ 4 \end{bmatrix}$ の形の空欄が出ます。$3$ 次元なら `Number of rows` を $3$ にするだけです。
...
これで `dotP(v,w)`、大きさの $\sqrt{\texttt{dotP}(v,\ v)}$、角の式まで、**打ち直しが $1$ 回で済みます**（[AHL 3.10b の GDC 第2節](ahl-3-10b.qmd#gdc-store)）。
...
**Radian のまま度がほしいときは $\dfrac{180}{\pi}$ を掛けます。** Degree 設定のままこれをやると、$121.3$ にさらに $\dfrac{180}{\pi}$ が掛かってしまいます（[AHL 3.10b の GDC 第4節](ahl-3-10b.qmd#gdc-angle)）。
...
絶対値の $\lvert\ \rvert$ は、**Math Templates** の中にあります。これで囲んでおくと、**$180^{\circ}$ から引く手間がなくなります**（[第7節](#lines)）。$70.3^{\circ}$ が直に返ります。
...
**(a)** $\overrightarrow{AB} = \mathbf{b} - \mathbf{a}$ です（[AHL 3.10b 第3節](ahl-3-10b.qmd#ab)）。

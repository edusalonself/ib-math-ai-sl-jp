...
![Reading a logistic model](img/ahl-2-9b-logistic.svg){#fig-ahl29b-logistic width=74%}

@fig-ahl29b-logistic が
...
**carrying capacity**（環境収容力）は、「その環境が支えられる上限」という意味です。@fig-ahl29b-logistic では $L = 2000$ です。
...
@fig-ahl29b-logistic では $L = 2000$、$f(0) = 200$ なので
...
$k$ が大きいほど、S 字が立ちます。**$L$ と $C$ が同じなら、上限に速く近づく**ということです。

$k$ は、途中の点を $1$ つ与えられて求めます。$L$ と $C$ が先に分かっていれば、あとは方程式を $1$ つ解くだけです（@exm-ahl29b-fit）。

いちばん急なのは、**ちょうど $\dfrac{L}{2}$ のところ**です。

$$
\frac{L}{1+Ce^{-kx}} = \frac{L}{2} \ \Longrightarrow \ Ce^{-kx} = 1 \ \Longrightarrow \ x = \frac{\ln C}{k}
$$ {#eq-ahl29b-half}

@fig-ahl29b-logistic なら $L = 2000$、$C = 9$、$k = 0.3$ なので、$\dfrac{\ln 9}{0.3} = 7.32$ の点（$P = 1000$）です。**$k$ が大きいほど、この時刻は早く来ます。**

この点を **point of inflexion**（変曲点）と呼びます。S 字はこの点について**点対称**で、上半分と下半分は同じ形です。求め方は [AHL 5.10](../05-calculus/ahl-5-10.qmd) です。

### 5. logistic と exponential のちがい {#vs-exp}

![A ceiling, or no ceiling](img/ahl-2-9b-vs-exp.svg){#fig-ahl29b-vsexp width=74%}

@fig-ahl29b-vsexp が、$P = \dfrac{2000}{1+9e^{-0.3t}}$ と $P = 200e^{0.3t}$ を重ねたものです。
...
![A piecewise model in context](img/ahl-2-9b-tariff.svg){#fig-ahl29b-tariff width=74%}

@fig-ahl29b-tariff が、携帯の料金です。$5$ GB までは $20$、そこからは $1$ GB ごとに $3$ ずつ増えます。式に書くと、次のようになります。
...
![Choosing $a$ so the graph joins up](img/ahl-2-9b-continuity.svg){#fig-ahl29b-continuity width=74%}

@fig-ahl29b-continuity が、その結果です。緑の線（$a = \dfrac{1}{4}$）は $(2,\ 3)$ でぴったりつながります。赤い線（$a = 1$）は $(2,\ 6)$ から始まってしまい、**$3$ だけとびます。**
...
同じ $20$ になります。だから @fig-ahl29b-tariff のように、**グラフはとばずにつながります。**
...
**(c)** `Explain` なので、英語で書きます。@fig-ahl29b-continuity の赤い線がその様子です。
...
$f1(x) = \dfrac{2000}{1 + 9 \times e^{-0.3 \times x}}$
...
@fig-ahl29b-vsexp が、まさにこの比較です。**はじめのうちは $2$ つがほとんど同じ**である点にも触れられると、なお良い答案になります（[第5節](#vs-exp)）。
...
## クリックすると開きます
**分母を見ると、$L$ に届かない理由が見えます。** $L > 0$、$C > 0$、$k > 0$ なので（公式集の条件です）、$Ce^{-kx}$ は正の数です。分母 $1 + Ce^{-kx}$ は**つねに $1$ より大きい**まま $1$ に近づくので

$$
0 < \frac{L}{1+Ce^{-kx}} < L
$$

です。**分母が $1$ ちょうどになることはない**ので、値が $L$ に届くこともありません（[第2節](#L)）。
...
ただし $y = 0$ のほうは、**$x$ を負にできる「関数として見たとき」の話**です。文脈の問題では $t \geq 0$ しか意味を持たないので、答案に書く漸近線は $y = L$ のほうです。**$2$ 本の漸近線にはさまれて、そのあいだを $1$ 度だけ上がる**——これが S 字の正体です。

**いちばん急な時刻 $\dfrac{\ln C}{k}$ が正になるのは $C > 1$ のときです**（[第4節](#k)）。$C < 1$ なら、はじめの値がもう $\dfrac{L}{2}$ を超えていて、いちばん急なところは $x = 0$ より前にあります。文脈の問題では、その場合「いちばん急なのは最初」と答えます。

...
- 変換の言葉と式を、[AHL 3.9](../03-geometry-and-trigonometry/ahl-3-9.qmd) の行列と結び付けられる。
:::
...
公式集の Topic 2 の HL の部分にあるのは、Prior learning の quadratic formula と、2.1、2.5、2.9 だけです。**変換の式は $1$ つも印刷されていません。**

いっぽう、[AHL 3.9](../03-geometry-and-trigonometry/ahl-3-9.qmd#booklet-six) の `Transformation matrices` の欄には、**行列の形で $6$ つ印刷されています。** そのうち reflection と $2$ つの stretch は、この項目の $y = -f(x)$、$y = f(-x)$、$y = p\,f(x)$、$y = f(qx)$ と同じ動きをします。**同じ動きなのに、行列の形でしか配られていません**（[第8節](#matrix)）。

残りの $2$ つ（rotation と enlargement）は、この項目には出てきません。平行移動は、どちらの形でも印刷されていません。

この項目で身につけるのは、式ではなく**向きと順序**です。$f(x-a)$ が右か左か、$f(qx)$ が $q$ 倍か $\dfrac{1}{q}$ 倍か——ここを外すと、その先の計算が全部ずれます。
:::
...
Guidance 欄には、次の $6$ つが書かれています。

> Students will be expected to be able to perform transformations on all functions from the SL and AHL section of this topic, and others in the context of modelling real-life situations.
...
**Reflections の $2$ 式については、Guidance 欄に対応する記述がありません。** そのぶん、どちらの軸がどちらの式かは、自分で確実にしておく必要があります（[第3節](#reflection)）。

Connections 欄には、$3$ つ挙がっています。

> **Other contexts:** Translating curves to reduce rounding errors for large values.

> **Links to other subjects:** Shifting of supply and demand curves (economics); electromagnetic induction (physics)
...
$2$ つの **Example** は、そのまま @exm-ahl28-order と @exm-ahl28-sine で扱います。

Guidance の $1$ 行目が言うとおり、**変換の相手は $x^2$ とはかぎりません。** [SL 2.5](../../ai-sl/02-functions/sl-2-5.qmd) の $6$ つのモデルにも、AHL 2.9 で足される関数にも、同じ操作をします。
:::
...
シラバスが扱うのは、次の $4$ 種類と、その組み合わせです。

![The four basic transformations](img/ahl-2-8-four.svg){#fig-ahl28-four width=100%}

@fig-ahl28-four の灰色の点線が、もとの $y = f(x)$ です。青や赤の線が、変換したあとの **image**（像）です。
...
@fig-ahl28-four の (a) が、その両方を一度にやった例です。$y = f(x-2)+1$ は、**右に $2$、上に $1$** 動かしたものです。もとのグラフの山の頂上 $(1,\ 3)$ が $(3,\ 4)$ に移っています。
...
@fig-ahl28-four の (b) です。赤い実線が $y = -f(x)$ で、**上下がひっくり返って**います。緑の点線が $y = f(-x)$ で、**左右がひっくり返って**います。
...
@eq-ahl28-vstretch の $p$ を **scale factor**（拡大率）といいます。**scale factor は正の大きさとして書きます。** @fig-ahl28-four の (c) が $p = 2$ の場合で、山の高さが $3$ から $6$ になっています。
...
@eq-ahl28-hstretch で $q = 2$ としたものが、@fig-ahl28-four の (d) です。$y = f(2x)$ は、**横に $\dfrac{1}{2}$ 倍**——つまり半分に縮んでいます。山の頂上は $x = 1$ から $x = 0.5$ に移りました。
...
$f(x) = \sin x$ で確かめてください。$\sin x$ の周期は $2\pi$ ですが、$\sin 2x$ の周期は $\pi$ です。**$1$ 周が短くなっています**（@fig-ahl28-order の (b)）。
...
**縦に $3$ 倍してから、上に $2$** です。順に書くと

$$
x^2 \ \xrightarrow{\ \times 3\ } \ 3x^2 \ \xrightarrow{\ +2\ } \ 3x^2+2
$$

順を入れかえると、どうなるでしょうか。**上に $2$ してから、縦に $3$ 倍**します。

$$
x^2 \ \xrightarrow{\ +2\ } \ x^2+2 \ \xrightarrow{\ \times 3\ } \ 3\left(x^2+2\right) = 3x^2+6
$$

![Order matters, and two stretches at once](img/ahl-2-8-order.svg){#fig-ahl28-order width=100%}

@fig-ahl28-order の (a) が、その $2$ 本です。$3x^2+2$ と $3x^2+6$ で、**つねに $4$ だけ離れています。** 上に動かした $2$ も一緒に $3$ 倍されてしまうからです。
...
**中の操作は、そう単純ではありません。** かっこの中を $q(x-a)$ の形に**因数分解してから**読みます。

$$
y = f\bigl(q(x-a)\bigr) \quad \text{は、横に } \frac{1}{q} \text{ 倍してから、右に } a \qquad (q > 0)
$$ {#eq-ahl28-inside}

たとえば $y = f(2x-6)$ なら、$2x-6 = 2(x-3)$ なので、**横に $\dfrac{1}{2}$ 倍してから、右に $3$** です。

::: {.callout-warning}
## $f(2x-6)$ には、正しい道すじが $2$ 通りあります。まちがいは「縮めてから右に $6$」です
同じ $f(2x-6)$ を作る道すじは、$2$ 通りあります。

- **右に $6$ 動かしてから、横に $\dfrac{1}{2}$ 倍** … $f(x) \to f(x-6) \to f(2x-6)$
- **横に $\dfrac{1}{2}$ 倍してから、右に $3$** … $f(x) \to f(2x) \to f\bigl(2(x-3)\bigr) = f(2x-6)$
...
まちがいなのは、**縮めてから右に $6$** です。これは $f\bigl(2(x-6)\bigr) = f(2x-12)$ で、**別のグラフ**になります（演習 $7$ の (b)）。
:::
...
@fig-ahl28-order の (b) が、その結果です。**amplitude（振幅）が $4$、period（周期）が $\pi$** になっています。

### 8. 行列（AHL 3.9）との対応 {#matrix}

同じ変換を、[AHL 3.9](../03-geometry-and-trigonometry/ahl-3-9.qmd) では**行列**で書きます。**中身は同じ話**なので、対応させておくと両方が軽くなります。

| AHL 2.8 の式 | AHL 3.9 の行列 | 一致するか |
|:--|:--|:--|
| $y = -f(x)$ | $\begin{pmatrix} 1 & 0 \\ 0 & -1 \end{pmatrix}$（$x$ 軸で reflection） | そのまま |
| $y = f(-x)$ | $\begin{pmatrix} -1 & 0 \\ 0 & 1 \end{pmatrix}$（$y$ 軸で reflection） | そのまま |
| $y = p\,f(x)$ | $\begin{pmatrix} 1 & 0 \\ 0 & p \end{pmatrix}$（$p>0$ なら vertical stretch, scale factor $p$） | そのまま |
| $y = f(qx)$ | $\begin{pmatrix} k & 0 \\ 0 & 1 \end{pmatrix}$（$k>0$ なら horizontal stretch, scale factor $k$） | **$k = \dfrac{1}{q}$** |
| $y = f(x-a)+b$ | 行列の外に $\begin{pmatrix} a \\ b \end{pmatrix}$ を足す | そのまま |

: AHL 2.8 と AHL 3.9 {#tbl-ahl28-matrix}

いちばん下の行だけは、公式集にありません。translation は transformation matrix の中に入らないからです（[AHL 3.9 の第4節](../03-geometry-and-trigonometry/ahl-3-9.qmd#translation)）。

@tbl-ahl28-matrix で**式と行列がずれるのは、横の拡大の $1$ 行だけです。** 行列のほうは $x$ 座標にそのまま $k$ をかけるので、**$k$ がそのまま scale factor** です。関数のほうは $x$ に $q$ をかけて $f$ に**入れる**ので、グラフは $\dfrac{1}{q}$ 倍になります。

::: {.callout-tip}
## 順序の書き方も、$2$ つの単元で逆に見えます
行列では、**あとにやるほうを左**に書きます（[AHL 3.9](../03-geometry-and-trigonometry/ahl-3-9.qmd#composition)）。

$$
Q\bigl(P\mathbf{x}\bigr) = (QP)\,\mathbf{x}
$$

関数の式では、**$f$ に近い操作が先**です。外側なら $3f(x)+2$ の $\times 3$、内側なら $f\bigl(2(x-3)\bigr)$ の $\times 2$ が先です。

**行列は「点に近いほうが先」、関数の式は「$f$ に近いほうが先」**——ここだけ逆になります。$f\bigl(2(x-3)\bigr)$ で $x$ のとなりに書いてある $-3$ は、**あと**にやる操作です。「$x$ に近いほうが先」と読みかえると、内側の順序を取りちがえます。
:::
...
$$
x^{2} \ \xrightarrow{\ \text{縦に } 3 \text{ 倍}\ } \ 3x^{2} \ \xrightarrow{\ \text{上に } 2\ } \ 3x^{2}+2
$$
...
$$
x^{2} \ \xrightarrow{\ \text{上に } 2\ } \ x^{2}+2 \ \xrightarrow{\ \text{縦に } 3 \text{ 倍}\ } \ 3\left(x^{2}+2\right) = 3x^{2}+6
$$
...
差は $3x^{2}+6-\left(3x^{2}+2\right) = 4$ で、$x$ によりません（@fig-ahl28-order の (a)）。この $4$ は、**上に動かした $2$ が余分に $3$ 倍された分**です。
...
@fig-ahl28-order の (b) で、山の高さが $4$、$1$ 周の長さが $\pi$ になっていることを確かめてください。
...
$y = f(2x)$ は **scale factor $\dfrac{1}{2}$** の horizontal stretch です。$2$ ではありません。
...
- 合成のときは、**$2$ つの順序で $2$ 本かいて重ねる。** 重ならなければ、順序が効いています（@fig-ahl28-order の (a)）

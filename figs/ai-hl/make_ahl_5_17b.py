...
ax.plot([0], [0], "o", color=INK, ms=9, zorder=9)
ax.set_title("(c)  $\\lambda = \\pm 2i$  (purely imaginary)\nclosed CIRCLES",
...
ax.plot([0], [0], "o", color=INK, ms=9, zorder=9)
ax.set_title("(d)  $\\lambda = \\pm 2i$  as well\nclosed ELLIPSES",
             fontsize=12.5, color=GOLD, pad=10)

fig.suptitle("Complex eigenvalues: the path turns", fontsize=15, y=0.995)
fig.text(0.5, -0.015,
         "The IMAGINARY part makes the path go round.   The REAL part decides "
         "whether it also grows (out), shrinks (in), or neither (a closed "
         "curve).   Panels (c) and (d) have the SAME eigenvalues.",
         fontsize=12.5, ha="center", color=INK)
fig.tight_layout(rect=(0, 0, 1, 0.972))
save(fig, "ahl-5-17b-cases.svg")

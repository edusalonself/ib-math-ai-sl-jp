...
# 2026-09: 重複していた「底の条件」の囲みと第2節を、#conditions の囲み 1 つに統合
has("3 つの法則の直後に条件の囲みがある",
    "::: {#conditions .callout-warning}\n## 使える条件は $a > 0$、$a \\neq 1$、$x > 0$、$y > 0$ です")
hasnt("旧: 重複していた「底の条件」の囲みが残っていない",
      "## 底の条件は $a > 0$ と $a \\neq 1$ の $2$ つです")
has("条件をその場で式にしている",
    "a > 0, \\qquad a \\neq 1, \\qquad x > 0, \\qquad y > 0")
has("条件の式にラベルがある", "$$ {#eq-ahl19-conditions}")
hasnt("旧: 独立した第2節の見出しが残っていない",

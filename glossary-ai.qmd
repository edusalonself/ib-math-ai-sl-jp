...
この表は **AI 専用**です。AA（Analysis and Approaches）には proof や mathematical induction のように AI では出てこない用語があるので、AA の対訳表（`glossary-aa.qmd`）は別に用意します。AI HL の用語は、書き上がりしだいこの表に **HL** を付けて足していきます。
:::
...
| estimation | 概算、見積もり | |
| absolute value | 絶対値 | $\lvert\ \rvert$ の記号。中身のマイナスを外す。電卓はテンプレートか `abs()` |
...
| imaginary | 虚数の | AI SL では扱わない。**HL** AHL 1.12a |
| discriminant | 判別式 | **HL** $\Delta = b^{2}-4ac$。公式集の 1.12 の欄 |
| imaginary unit | 虚数単位 | **HL** $i$。$i^{2} = -1$ と定義する |
| complex number | 複素数 | **HL** $a + bi$ の形の数 |
| Cartesian form | 直交形式 | **HL** $z = a + bi$ の書き方。公式集の 1.12 の欄 |
| real part | 実部 | **HL** $a+bi$ の $a$。$i$ は含まない実数 |
| imaginary part | 虚部 | **HL** $a+bi$ の $b$。$bi$ ではない |
| purely imaginary | 純虚数 | **HL** real part が $0$ |
| conjugate | 共役複素数 | **HL** $z^{*} = a - bi$。虚部の符号だけ変える |
| conjugate pair | 共役な組 | **HL** $a+bi$ と $a-bi$。実数係数なら必ずセット |
| complex plane | 複素平面 | **HL** 横が real axis、縦が imaginary axis |
| Argand diagram | アルガン図 | **HL** 複素数を点として書いた図。AHL 1.12b |
| real axis / imaginary axis | 実軸／虚軸 | **HL** 目盛りはどちらも実数 |
| modulus (of a complex number) | 絶対値 | **HL** $\lvert z \rvert = \sqrt{a^{2}+b^{2}}$。原点からの距離。公式集にない |
| argument | 偏角 | **HL** $\arg z$。正の実軸から測った角。象限の確認が要る |
| principal argument | 主偏角 | **HL** $-\pi < \theta \leq \pi$ の範囲にとったもの |
| quadrant | 象限 | **HL** $\arctan$ の値を直すかどうかを決める |
| impedance | インピーダンス | **HL** 交流回路で $Z = R + Xi$ と書く量 |
| phase angle | 位相角 | **HL** $\arg Z$。工学では度で言うことが多い |
| polar form | 極形式 | **HL** $r(\cos\theta + i\sin\theta)$。公式集の 1.13 の欄 |
| modulus–argument form | 極形式 | **HL** polar form の別名。公式集の見出しはこちら |
| exponential form | 指数形式 | **HL** $re^{i\theta}$。Euler form ともいう |
| Euler form | オイラー形式 | **HL** exponential form の別名。Guidance に出てくる |
| cis | シス | **HL** $\operatorname{cis}\theta = \cos\theta + i\sin\theta$ の略記 |
| integer power | 整数べき乗 | **HL** $z^{n} = r^{n}\operatorname{cis}(n\theta)$ |
| rotation | 回転 | **HL** かけ算の幾何的な意味の半分 |
| stretch | 拡大・縮小 | **HL** かけ算の幾何的な意味のもう半分 |
| frequency | 周波数 | **HL** $\omega$ が同じ波どうしでないと合成できない |
| phase shift | 位相のずれ | **HL** $A\cos(\omega t + B)$ の $B$（角）。時間の移動量は $-\dfrac{B}{\omega}$ 秒 |
| out of phase | 位相がずれている | **HL** ずれていると、振幅は単純な和にならない |
| alternating current (AC) | 交流 | **HL** シラバスの Guidance に出てくる文脈 |
| to cancel | 打ち消し合う | **HL** 逆向きの複素数は足すと小さくなる |
| rational exponent | 有理数の指数 | **HL** AHL 1.10。$a^{\frac{m}{n}}$ の形 |
| radical / surd | 根号 | **HL** $\sqrt[n]{a}$。指数の形に直してから計算する |
| $n$th root | $n$ 乗根 | **HL** $a^{\frac{1}{n}}$。$n$ 乗すると $a$ になる正の数 |
| numerator / denominator | 分子／分母 | **HL** 指数では、分母が根、分子が累乗 |
| to simplify | 簡単にする | **HL** 指定された form まで整理する |
| partial sum | 部分和 | **HL** AHL 1.11。最初の $n$ 項の和 $S_n$ |
| sum to infinity | 無限級数の和 | **HL** $S_{\infty}$。$\lvert r \rvert < 1$ のときだけ存在する |
| to converge | 収束する | **HL** 部分和がひとつの値に近づくこと |
| to diverge | 発散する | **HL** 落ち着き先を持たないこと。$\lvert r \rvert \geq 1$ |
| limit | 極限 | **HL** $n \to \infty$ での行き先。SL 5.1 の考え方 |
| to approach | 近づく | **HL** $\to$ の読み方。「等しくなる」ではない |
| to come to rest | 止まる | **HL** 弾むボールの問題に出る |
...
| scalar | 実数、スカラー | **HL** 「ただの数」。行列に掛けるとき（AHL 3.9）も、ベクトルに掛けるとき（AHL 3.10）も同じ言い方 |
...
| composite function | 合成関数 | **HL** AHL 2.7。$(f \circ g)(x) = f(g(x))$。先に働くのは $g$ のほう |
| composed with | 〜と合成した | **HL** $\circ$ の読み方。$f \circ g$ は「$f$ composed with $g$」 |
| domain restriction | 定義域の制限 | **HL** AHL 2.7。逆関数を持たせるために、one-to-one になるまで domain を絞ること。$f(x)=(x-3)^2-2$ のような放物線では、頂点の左右どちらか一方に制限する |
| to restrict | 制限する | **HL** `restricted to $x \geq 3$` のように使う |
| to verify | 確かめる | **HL** 計算して合うことを示す。$(f \circ f^{-1})(x) = x$ など |
| finished width | 仕上がりの幅 | 処理を全部終えたあとの幅 |
| border | 縁・へり | 写真や紙のまわりに付ける部分 |
| to convert | 変換する | 単位や表し方を変える |
| to linearize | 線形化する | **HL** AHL 2.10。$\log$ を取って直線に直すこと |
| semi-log graph | 片対数グラフ | **HL** 縦だけ $\log$。直線なら exponential |
| log-log graph | 両対数グラフ | **HL** 縦も横も $\log$。直線なら power |
| power relationship | 累乗の関係 | **HL** $y = ax^{n}$。log-log の傾きが $n$ |
| exponential relationship | 指数の関係 | **HL** $y = ka^{x}$。semi-log の傾きが $\log a$ |
| growth factor | 増加倍率 | **HL** $1$ 目盛りあたり何倍か。semi-log の傾きは $\log$ を取った値 |
| best-fit straight line | 最も当てはまる直線 | **HL** 線形化したあとにかける回帰直線 |
| manageable scale | 扱いやすい目盛り | **HL** 桁がちがうデータに対数目盛りを使う理由 |
| wide range of values | 値の幅が広い | **HL** 対数目盛りが要る場面 |
| rate of growth | 増え方の割合 | **HL** 絶対値ではなく、何倍ずつ増えるか |
| absolute value | 絶対的な値 | **HL** ここでは「数そのもの」の意味。$\lvert x \rvert$ ではありません |
| degrees Fahrenheit | 華氏 | 日本では使いません。$32$ °F が $0$ °C |
| kelvin | ケルビン | 絶対温度。$0$ °C が $273.15$ K |
| transformation of a graph | グラフの変換 | **HL** AHL 2.8。式を書きかえてグラフを動かすこと |
| vertical stretch | 縦の拡大 | **HL** $y = p\,f(x)$。$y$ 座標だけ $p$ 倍。$x$ 軸は動きません |
| horizontal stretch | 横の拡大 | **HL** $y = f(qx)$。scale factor は $\dfrac{1}{q}$。$q$ ではありません |
| composite transformation | 合成変換 | **HL** AHL 2.8。$2$ つ以上を続けて行うこと。順序が効く場合があります |
| in the $x$ axis | $x$ 軸について | **HL** reflection の向き。式は $y = -f(x)$ |
| in the $y$ axis | $y$ 軸について | **HL** reflection の向き。式は $y = f(-x)$ |
| translation by the vector | 〜だけ平行移動 | **HL** $\begin{pmatrix} 3 \\ -2 \end{pmatrix}$ は右に $3$、下に $2$ |
| to describe fully | 完全に述べる | **HL** 変換の名前と scale factor（または向きと大きさ）の両方を書く |
| resulting graph | できあがるグラフ | 変換したあとのグラフ |
| in the opposite order | 逆の順で | 変換の順序を入れかえること |
...
| period | 周期 | $\dfrac{360}{b}$（度数法）。**HL** ではラジアンが既定で $\dfrac{2\pi}{b}$。公式集にありません |
...
| natural logarithmic model | 自然対数モデル | **HL** AHL 2.9a。$f(x) = a + b\ln x$。domain は $x > 0$ |
| logistic model | ロジスティックモデル | **HL** AHL 2.9b。$\dfrac{L}{1+Ce^{-kx}}$。上限 $L$ に近づいて止まる |
| carrying capacity | 環境収容力 | **HL** logistic の上限 $L$。horizontal asymptote。届くことはありません |
| restriction on the growth | 増え方の制限 | **HL** logistic を使う場面の言い方 |
| to level off | 頭打ちになる | **HL** 伸びが止まっていくこと |
| continuity | 連続性 | **HL** AHL 2.9b。境目で $2$ つの式が同じ値になること。厳密な定義は要りません |
| to ensure continuity | つながるようにする | **HL** `find a to ensure continuity` の形で出ます |
| the two pieces | $2$ つの部分 | **HL** piecewise の $2$ つの式のこと |
| radian measure | 弧度法 | **HL** AHL 3.7。HL の sinusoidal model はこちらが既定 |
| degree symbol | 度の記号 | $^{\circ}$。$\sin$ の中の角に付いていれば度数法 |
...
| degree measure | 度数法 | **HL** $^\circ$ の記号が付いているとき |
| convert | 換算する・直す | **HL** $180^{\circ} = \pi$ radians を使います |
| subtend / subtended at the centre | （中心に対して角を）張る | **HL** 「その弧を見込む中心角」のこと |
| arc length | 弧の長さ | **HL** $l = r\theta$（$\theta$ はラジアン） |
| area of a sector | 扇形の面積 | **HL** $A = \dfrac{1}{2}r^{2}\theta$（$\theta$ はラジアン） |
| exact multiple of π | $\pi$ の正確な倍数 | **HL** 小数にせず $\dfrac{4\pi}{5}$ の形で止めます |
| in terms of π | $\pi$ を残した形で | **HL** 同じ指示です |
| full turn / complete turn | $1$ 周 | **HL** $2\pi$ radians $= 360^\circ$ |
| pendulum | 振り子 | **HL** ひもの長さが半径になります |
| bob | 振り子のおもり | **HL** 動く道すじは円弧 |
| sprinkler | スプリンクラー | **HL** 扇形に水をまく設定でよく出ます |
| unit circle | 単位円 | **HL** AHL 3.8。原点中心、半径 $1$ の円 |
| quadrant | 象限 | **HL** 単位円を $4$ つに分けた領域。符号がここで決まる |
| Pythagorean identity | ピタゴラスの恒等式 | **HL** $\cos^{2}\theta+\sin^{2}\theta = 1$。公式集にあります |
| identity | 恒等式 | **HL** どの $\theta$ でも成り立つ等式。equation とは違います |
| ambiguous case | 二通りになる場合 | **HL** sine rule で角が $2$ 通り出ること。AI **SL では出ません** |
| acute / obtuse | 鋭角の／鈍角の | **HL** $90^\circ$ より小さい／大きい |
| to subtend | （角を）張る | **HL** 三角形の設定でよく使われます |
| anticlockwise | 反時計回り | **HL** 角を測る向き。時計回りは clockwise |
| coordinate | 座標 | **HL** $\cos\theta$ は $x$ 座標、$\sin\theta$ は $y$ 座標 |
| finite interval | 有限区間 | **HL** $0 \leq x \leq 2\pi$ のような範囲。解の個数を決めます |
| all the values | すべての値 | **HL** 区間内の解を $1$ つ残らず書く、という指示 |
| greenhouse | 温室 | **HL** 周期モデルの文脈でよく出ます |
| harbour | 港 | **HL** 潮の満ち引きのモデルでよく出ます |
| vector | ベクトル | **HL** AHL 3.10。大きさと向きを持つもの。AI **SL では出ません** |
| directed line segment | 有向線分 | **HL** 向きの付いた矢印。ベクトルの図での書き方 |
| component（ベクトルの） | 成分 | **HL** 「右にいくつ、上にいくつ」の数 |
| column representation | 列ベクトルの書き方 | **HL** $\begin{pmatrix} 4 \\ 3 \end{pmatrix}$ のように縦に並べる形 |
| base vectors | 基本ベクトル | **HL** $\mathbf{i}$、$\mathbf{j}$、$\mathbf{k}$。各軸の向きの長さ $1$ のベクトル |
| unit vector | 単位ベクトル | **HL** 長さが $1$ のベクトル |
| zero vector | 零ベクトル | **HL** $\mathbf{0}$。成分が全部 $0$。**向きを持ちません** |
| scalar multiple | スカラー倍 | **HL** $k\mathbf{a}$。全部の成分に $k$ を掛ける |
| parallel（ベクトルが） | 平行 | **HL** $\mathbf{a} = k\mathbf{b}$ と書けること。**同じ向きとはかぎりません** |
| resultant | 合成ベクトル、合力 | **HL** いくつかのベクトルを**全部足したもの**。新しい計算ではありません |
| leg（行程の） | 行程、区間 | **HL** 「$3$ 本に分けて飛んだ」の $1$ 本ずつ |
| speed | 速さ | **HL** scalar。数だけ。velocity とは別 |
| velocity | 速度 | **HL** vector。数と向き。speed とは別 |
| force | 力 | **HL** vector。「下向きに $30$ N」のように向きが要る |
| in the form | 〜の形で | **HL** $a\mathbf{i}+b\mathbf{j}$ と指定されたら、列ベクトルで答えません |
| magnitude | 大きさ | **HL** AHL 3.10。$\lvert \mathbf{v} \rvert$。矢印の長さ。**scalar** |
| position vector | 位置ベクトル | **HL** 原点から点へ向かうベクトル。$\overrightarrow{OA} = \mathbf{a}$ |
| normalize | 正規化する、単位化する | **HL** 長さを $1$ にそろえること。$\dfrac{\mathbf{v}}{\lvert \mathbf{v} \rvert}$ |
| rescale | 長さを付け替える | **HL** 単位ベクトルにしてから、ほしい長さを掛ける |
| bearing | 方位 | **HL** 北から東回りに測った角。$3$ 桁で書きます |
| ground speed | 対地速度 | **HL** 風を入れた実際の速さ。合成速度の大きさ |
| current（潮流の） | 潮流 | **HL** 船を流す向きと速さ。ベクトルで足します |
| exact value | 正確な値 | **HL** $\sqrt{34}$ のまま。小数にしません |
| to the nearest degree | $1$ 度の単位で | **HL** 方位でよく出る指示 |
| vector equation (of a line) | 直線のベクトル方程式 | **HL** AHL 3.11。$\mathbf{r} = \mathbf{a} + \lambda\mathbf{b}$ |
| direction vector | 方向ベクトル | **HL** 直線の向きを表すベクトル。$\mathbf{0}$ ではいけません |
| parameter | 媒介変数 | **HL** $\lambda$。直線上のどこかを指すだけの数 |
| parametric form | 媒介変数表示 | **HL** 成分ごとに $1$ 行ずつ書いた形 |
| lies on the line | 直線上にある | **HL** $1$ つの $\lambda$ が**全成分**で成り立つこと |
| point of intersection | 交点 | **HL** $2$ 直線が同じ点に来るところ |
| skew | ねじれの位置 | **HL** $3$ 次元で、平行でも交わりもしないこと。公式は問われません |
| constant velocity | 等速度 | **HL** AHL 3.12。向きも速さも変わらない動き |
| particle | 粒子、物体 | **HL** 大きさを無視して $1$ 点として扱うもの |
| path | 経路 | **HL** 通る線。等速度なら直線（の一部） |
| collide / collision | 衝突する／衝突 | **HL** **同じ場所に同じ時刻**にいること |
| relative position | 相対位置 | **HL** $\overrightarrow{AB} = \mathbf{r}_B - \mathbf{r}_A$ |
| relative velocity | 相対速度 | **HL** $\mathbf{v}_B - \mathbf{v}_A$ |
| closest approach | 最接近 | **HL** $2$ 物体の距離が最小になるとき |
| ground speed | 対地速度 | **HL** 風を入れた実際の速さ（AHL 3.10b と同じ語） |
| variable velocity | 変わる速度 | **HL** AHL 3.12。$\mathbf{v}$ が $t$ の式になっている運動 |
| projectile motion | 放物運動 | **HL** 加速度が下向きの定数。$\mathbf{v}$ が $t$ の $1$ 次式 |
| circular motion | 円運動 | **HL** $\mathbf{r} = (R\cos\omega t,\ R\sin\omega t)$。速さは一定 |
| time shift | 時間のずらし | **HL** $f(t-a)$。$a$ だけ遅れて同じことが起こる |
| greatest height | 最高の高さ | **HL** 加速度が下向きなら $v_y = 0$ のとき |
| ground（放物運動の） | 地面 | **HL** `returns to the ground` は $y = 0$ のこと |
| scalar product | スカラー積、内積 | **HL** AHL 3.13。$\mathbf{v} \cdot \mathbf{w}$。答えは**数**。ベクトルではありません |
| dot product | ドット積 | **HL** scalar product の別名。**同じもの**です |
| the angle between two vectors | $2$ ベクトルのなす角 | **HL** 同じ点から描いた $2$ 本のあいだの角。$0^\circ$ 〜 $180^\circ$ で $1$ つに決まる |
| the acute angle between two lines | $2$ 直線のなす鋭角 | **HL** 直線には向きがないので、鋭角のほうを答えます |
| perpendicular（ベクトルが） | 垂直 | **HL** $\mathbf{v} \cdot \mathbf{w} = 0$。ただし**両方 $\mathbf{0}$ でないとき** |
| non-zero | $\mathbf{0}$ でない | **HL** シラバスがわざわざ書いています。角を考えられる条件 |
| the component of a in the direction of b | $\mathbf{b}$ の向きの $\mathbf{a}$ の成分 | **HL** $\dfrac{\mathbf{a} \cdot \mathbf{b}}{\lvert \mathbf{b} \rvert}$。割るのは **`in the direction of` のほう** |
| projection | 射影、影 | **HL** $\mathbf{b}$ の線の上に落とした $\mathbf{a}$ の影 |
| work done | 仕事 | **HL** $W = \mathbf{F} \cdot \mathbf{d}$。式は問題文で与えられます |
| displacement | 変位 | **HL** どこからどこへ動いたか。ここでは vector（AHL 5.13 では $1$ 次元の $s$） |
| vector product | ベクトル積、外積 | **HL** AHL 3.13。$\mathbf{v} \times \mathbf{w}$。答えは**ベクトル**。$3$ 次元だけ |
| cross product | クロス積 | **HL** vector product の別名。**同じもの**です |
| unit normal vector | 単位法線ベクトル | **HL** $\mathbf{n}$。長さ $1$ で、その平面に垂直 |
| normal（面に対する） | 法線 | **HL** 「垂直」の意味。normal distribution とは無関係です |
| right-hand screw rule | 右ねじの規則 | **HL** $\mathbf{v}$ から $\mathbf{w}$ へ回すときの、右手の親指の向き |
| parallel（ベクトルが、外積で） | 平行 | **HL** $\mathbf{v} \times \mathbf{w} = \mathbf{0}$。**ベクトル**の $\mathbf{0}$ です |
| parallelogram | 平行四辺形 | **HL** 面積は $\lvert \mathbf{v} \times \mathbf{w} \rvert$ |
| adjacent sides | 隣り合う $2$ 辺 | **HL** 同じ頂点から出ている $2$ 本 |
| area of a parallelogram | 平行四辺形の面積 | **HL** 公式集にあります。三角形はその半分 |
| the component of a acting perpendicular to b | $\mathbf{b}$ に垂直な $\mathbf{a}$ の成分 | **HL** $\dfrac{\lvert \mathbf{a} \times \mathbf{b} \rvert}{\lvert \mathbf{b} \rvert}$。**負になりません** |
| torque | トルク、回す力 | **HL** シラバスの Connections 欄。外積の使い道の例 |
| in exact form | 正確な形で | **HL** $10\sqrt{2}$ のまま。$14.1$ にしません |
...
| invariant | 不変の | **HL** 動かない点や直線のこと。AHL 2.8 では stretch のときの軸 |
...
| kinematics | 運動力学 | **HL** AHL 5.13。SL では出題されない |
...
| changes direction | 向きを変える | **HL** $v$ の**符号**が変わる |
...
| slope field | 方向場 | **HL** AHL 5.15。各点に傾きの**短い線分**をかいた図。「傾きを表す場」の意味 |
| direction field | 方向場 | **HL** slope field の別名。同じもの |
...
| logistic model | ロジスティックモデル | **HL** $\dfrac{dP}{dt} = kP\left(1 - \dfrac{P}{L}\right)$。上限で止まる。解いた形は AHL 2.9b |
...
| to overestimate | 大きすぎる値を出す | **HL** 解曲線が concave-down のとき |
| to underestimate | 小さすぎる値を出す | **HL** 解曲線が concave-up のとき |
| coupled system | 連立系 | **HL** $\dfrac{dx}{dt}$ と $\dfrac{dy}{dt}$ の $2$ 本 |
| predator–prey model | 捕食者・被食者モデル | **HL** $xy$ の項が「出会い」を表す |
| equilibrium point | 平衡点 | **HL** 両方の変化率が $0$ になる**点** |
| trajectory | 軌道 | **HL** $(x,\ y)$ 平面に描いた経路 |
| spreadsheet | 表計算（スプレッドシート） | **HL** シラバスが Euler 法で使うよう指示 |
| trace | 対角の和 | **HL** $\lambda$ の**和**に等しい。$\det$ は $\lambda$ の**積** |
| phase portrait | 相図 | **HL** AHL 5.17。$(x,\ y)$ 平面に軌道を描いた図 |
| node | 結節点 | **HL** 固有値が実数で同符号。原点から離れる／近づく |
| stable / unstable | 安定／不安定 | **HL** 原点に近づくか、離れるか |
| saddle point | 鞍点 | **HL** 固有値が実数で**異符号**。固有ベクトルの向きによって、近づくものと離れるものがある |
| spiral | 渦巻き | **HL** 固有値が**複素数**のとき |
| real part | 実部 | **HL** AHL 1.12a の $a$。AHL 5.17b では、近づくか離れるかを決める |
| imaginary part | 虚部 | **HL** AHL 1.12a の $b$。AHL 5.17b では、回る速さを決める |
| purely imaginary | 純虚数 | **HL** 実部が $0$。AHL 5.17b では軌道が**円または楕円** |
| conjugate pair | 共役な組 | **HL** $a+bi$ と $a-bi$。実数係数なら必ずセットで出る |
...
| velocity | 速度 | **HL** $\dfrac{dx}{dt}$。相図の**縦軸** |
...
| half-life | 半減期 | **HL** AHL 2.9a。半分になるまでの時間。最初の量によらない。$T = \dfrac{\ln 2}{k}$ |
...
| derivative | 導関数 | 数学II |
| first derivative | 第1次導関数 | $f'$。SL で扱うのはこれだけ |
| second derivative | 2階導関数 | **HL** AHL 5.10。$f''(x)$、$\dfrac{d^{2}y}{dx^{2}}$、$\ddot{x}$ |
| second derivative test | 2階導関数による判定 | **HL** $f''<0$ なら maximum、$f''>0$ なら minimum |
| concave-up | 下に凸 | **HL** $f''>0$。$\cup$ の形。シラバスの言葉 |
| concave-down | 上に凸 | **HL** $f''<0$。$\cap$ の形。シラバスの言葉 |
| concavity | 凸性 | **HL** $\cup$ か $\cap$ か、という曲がり方 |
| point of inflexion | 変曲点 | **HL** concavity が変わる点。AI HL では意味が分かれば十分 |
| the nature of (a point) | その点が何であるか | maximum か minimum かを答えよ、という意味 |
| to occur at | 〜のところで起こる | `stationary points occur at x = 2` |
| to cool | 冷める | 温度のモデルでよく出ます |
| outbreak | （感染症の）流行 | 増え方の解釈を問う文脈 |
| giving a reason | 理由を示して | 答えだけでは点になりません |
...
| standard integrals | 基本の積分 | **HL** 公式集 5.11 の欄の $5$ つ |
| integration by inspection | 微分の逆から予想する積分 | **HL** AHL 5.11b。答えを予想して微分し、定数を合わせる |
| integrand | 被積分関数 | $\displaystyle\int f(x)\,dx$ の $f(x)$ |
| modulus | 絶対値 | $\lvert x \rvert$。$\ln\lvert x \rvert$ で使います |
| exact value | 正確な値 | 小数に直さない。$\ln 2$、$\dfrac{83}{12}$ のまま書く |
| disc | 円板 | **HL** 回転体を切った薄い円。AHL 5.12b |
| solid of revolution | 回転体 | **HL** 曲線を軸のまわりに回してできる立体 |
| volume of revolution | 回転体の体積 | **HL** AHL 5.12b |
| region enclosed by | 〜で囲まれた領域 | どの線で囲まれているかを全部読む |
...
| composite function | 合成関数 | **HL** 「中身」のある関数。$\sin 4x$ など。記号は AHL 2.7 |
| related rates of change | 関連する変化率 | **HL** AHL 5.9b。同時に変わる2つの量 |
...
| kinematics | 運動力学 | シラバスは「SL の試験では出さない」と明記。**HL** AHL 5.13 |
...
| **Verify** | 結果が正しいことを示す根拠を書く。別のやり方で出し直すことが多い |
| **Identify** | どれかを指し示す。`Identify the error` なら**どこが違うかを言葉で書く** |

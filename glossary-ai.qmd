...
| phasor | フェーザ | **HL** 正弦波に対応させる複素数 $A\operatorname{cis}\phi$ |
...
| leading / lagging | 進み／遅れ | **HL** $B > 0$ なら進み、$B < 0$ なら遅れ |
| alternating current (AC) | 交流 | **HL** シラバスの Guidance に出てくる文脈 |
| to cancel | 打ち消し合う | **HL** 逆向きの phasor は足すと小さくなる |
| rational exponent | 有理数の指数 | **HL** AHL 1.10。$a^{m/n}$ の形 |
| radical / surd | 根号 | **HL** $\sqrt[n]{a}$。指数の形に直してから計算する |
| $n$th root | $n$ 乗根 | **HL** $a^{1/n}$。$n$ 乗すると $a$ になる正の数 |
...
| skew | ねじれの位置の | **HL** $3$ 次元で、平行でも交わりもしないこと。公式は問われません |
...
| integration by inspection | 見て積分する方法 | **HL** AHL 5.11b。答えを予想して微分し、定数を合わせる |

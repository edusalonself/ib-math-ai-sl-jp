...
@fig-sl32-labelling の右がそれです。角 $A$ の向かいが $a$、角 $B$ の向かいが $b$、角 $C$ の向かいが $c$ です。
...
**求めたい辺 $c$ の向かいにある角が $C$**、つまり **$a$ と $b$ にはさまれた角**です。この対応が崩れると、答えが合いません。
...
中学で習った $\dfrac{1}{2} \times$ 底辺 $\times$ 高さ も、もちろん使えます。**高さが分かっているときは、そちらのほうが速い**です。
:::
...
「面積が $50$ になる角は？」のような問題は、`nSolve` が速いです。

```
nSolve(0.5*9*14*sin(x) = 50, x)   →   52.528...
```

`nSolve` は `menu → Algebra → Numerical Solve` です（SL 1.8）。

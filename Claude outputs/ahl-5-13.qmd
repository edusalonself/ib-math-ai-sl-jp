...
# AHL 5.13 — Kinematics（運動の記述） {#sec-ahl-5-13}
...
::: {.callout-important}
## SL で学んだ微分と積分を、動く物体に使います
新しい微分の公式も、新しい積分の公式も出てきません。使うのは

- [SL 5.3](../../ai-sl/05-calculus/sl-5-3.qmd) の微分
- [SL 5.5](../../ai-sl/05-calculus/sl-5-5.qmd) の積分と定積分

の $2$ つです。**新しいのは、$s$、$v$、$a$ という $3$ つの量が微分でつながっている、という見方**のほうです。

シラバスは、この項目に次の注記を付けています。

> **Links to other subjects:** Kinematics (physics).

Physics の kinematics とつながる項目です。ただし、**AI の試験で問われるのは計算と解釈**です。物理の公式を暗記していなくても解けます。
:::

::: {.callout-note}
## SL では出ないと書かれていた内容です
[SL 5.7](../../ai-sl/05-calculus/sl-5-7.qmd) で、シラバスのこの一行を見ました。
...
$$
a = \frac{dv}{dt} = \frac{d^{2}s}{dt^{2}} = v\frac{dv}{ds}
$$ {#eq-ahl513-acc}
...
です。$a = 0$ を解くと、**いちばん速い（または遅い）瞬間**が出てきます。別のものです。
:::
...
公式集の $1$ 行目にある、$3$ 番目の書き方です（@eq-ahl513-acc）。
...
```
Numerical Integral:  下 0  上 6  式 5-e^(0.4x)  変数 x
```

$$
\int_{0}^{6} v\,dt = 4.94 \ \text{m}
$$

**(c)** total distance は $|v|$ の定積分です。

```
Numerical Integral:  下 0  上 6  式 abs(5-e^(0.4x))  変数 x
```
...
どちらも `Numerical Integral` です（[SL 5.5](../../ai-sl/05-calculus/sl-5-5.qmd#calc)）。
...
$v = t^{2} - 4t + 3$ を $t = 3$ で微分すると $2$。自分で作った $a = 2t - 4$ に $t = 3$ を入れても $2$ ✓
...
Numerical Integral:  下 0  上 4  式 abs(x*sin(x))  変数 x
```

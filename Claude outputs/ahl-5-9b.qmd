...
**書き直しで済むのは、$x$ のべき乗どうしの分数だけ**です。$\dfrac{e^{x}}{x}$ のように分子に $e^{x}$ や $\sin x$ が入っていると、書き直しても product rule が要るので、手間は変わりません。分母に $x$ 以外のものが混じっている（$x^{2}+1$、$e^{x}+1$ など）ときは、**書き直せないので quotient rule だけ**です。
...
1. **分数の形か** → はい なら quotient rule（ただし分子も分母も $x$ のべき乗だけなら、書き直せばルールが要りません。[第4節](#avoid)）

...
Guidance 欄には $2$ 行あります。

> $(f \circ f^{-1})(x) = (f^{-1} \circ f)(x) = x$.

> **Example:** $f(x) = (x-3)^2 - 2$ has an inverse if the domain is restricted to $x \geq 3$ or to $x \leq 3$.

Connections 欄には、何も挙がっていません。

AHL の Topic 2 全体の Recommended teaching hours は $11$ 時間です。

**Guidance の Example は、シラバスが名指しした関数**です。@exm-ahl27-restrict でそのまま扱います。

逆関数そのものは [SL 2.2](../../ai-sl/02-functions/sl-2-2.qmd#inverse) で一度出ています。SL では `Informal concept`（大まかな考え方）どまりでした。**HL で新しく足されるのは、$f^{-1}(x)$ の式を実際に求めることと、domain restriction です。**
:::
...
![Two machines in a row, and the two orders](img/ahl-2-7-composite.svg){#fig-ahl27-composite width=100%}

@fig-ahl27-composite の (a) を見てください。$f(x) = 2x+1$、$g(x) = x^{2}$ として、$3$ を入れます。
...
@fig-ahl27-composite の (b) が、この $2$ つのグラフです。**頂点の位置も、開き方も違います。**

読み方に迷ったら、@tbl-ahl27-notation に戻ってください。

$x = 3$ を入れれば、$2(9)+1 = 19$ と $(7)^{2} = 49$ で、(a) と合います。
...
**$f$ で進んで $f^{-1}$ で戻れば、元の場所に返る。** どちらの順でも同じです。

さきほどの $f(x) = \dfrac{3}{x-2}$、$f^{-1}(x) = \dfrac{3}{x}+2$ で確かめます。

$$
f\left(f^{-1}(x)\right) = f\left(\frac{3}{x}+2\right) = \frac{3}{\left(\frac{3}{x}+2\right)-2} = \frac{3}{\frac{3}{x}} = x
$$
...
となって、$x$ に戻ってしまいます。**この向きの検算は、枝の取りちがえを見つけられません。**
...
そこで **domain を片側だけに切ります。** 頂点は $(3, -2)$ なので、切れ目は $x = 3$ です。
...
$f$ の domain がすでに片側に切ってあれば（演習 $7$ の $x \geq 0$ のように）、$2$ つのうち片方は domain の外にあるので、そこで自動的に $1$ つに決まります。**切っていなければ、自分で切るしかありません。**

$\pm$ のうち片方だけを残すには、**もともとの $f$ の domain を片側に切っておく**しかありません。切ったあとの $f$ は one-to-one になり、$x$ について解いた式も $1$ つに決まります。

**domain restriction は、後付けの制約ではなく、逆関数が存在するための条件そのもの**です。
...
**(a)** と **(b)** で答えが違うことに注目してください（@fig-ahl27-composite の (a)）。
...
### 1. 関数を定義してから、合成を打つ {#gdc-define}

Calculator ページで、$2$ つの関数に名前を付けておきます。

```
menu → 1: Actions → 1: Define
```

を選ぶと、入力欄に `Define` の文字が出ます。そのあとに続きを打って、
...
の形にします（`Define` の部分は、メニューが入れてくれた分です）。あとは、そのまま打てます。

```
f(g(3))
```

$19$ と返ります。`g(f(3))` なら $49$ です。**順番を変えて打ってみると、違う数が返ることがすぐ確かめられます。**

### 2. 式は手で出し、電卓で検算する {#gdc-expand}

**TI-Nspire CX II は Numeric（非 CAS）です。** `f(g(x))` と打っても、$x$ に値が入っていなければ計算できず、エラーになります。**$2x^{2}+1$ という式を電卓が作ってくれることはありません。**

式は手で出したうえで、**いくつかの値で合うかを確かめる**のが電卓の役目です。
...
どちらも $33$ になれば、$(f \circ g)(x) = 2x^{2}+1$ が合っている見込みが高いといえます。$x = -1$、$x = 0.5$ など、**$2$ つか $3$ つの値**で試してください。

### 3. グラフで重ねて見る {#gdc-graph}

```
ctrl + doc → 2: Add Graphs
```

$f$ と $g$ を Define してあれば、グラフ画面でも使えます。

**[第2節](#gdc-expand) でエラーになった `f(g(x))` が、ここでは打てます。** Calculator ページの $x$ は「まだ何も入っていない文字」ですが、グラフの入力欄の $x$ は、電卓が点をかくたびに値を入れていく場所だからです。式を作っているのではなく、**値を次々に代入している**だけです。
...
**$2$ 本が完全に重なって見えれば、式が合っている見込みが高いといえます。** 画面に映っているのは $x$ の一部の範囲だけなので、「重なった＝証明できた」ではありません。

逆関数の検算にも使えます。$f3(x)=x$ を足して、$f(x)$ と $f^{-1}(x)$ が **$y = x$ について対称**になっているかを見ます（@fig-ahl27-inverse の (b)）。
...
::: {.callout-warning}
## 制限は、自分で打ちこまないと映りません
$f1(x)=(x-3)^2-2$ とだけ打つと、電卓は**放物線の全体**をかきます。制限のことは、電卓は何も知りません。

制限を見せたいときは、`piecewise` を使います。TI 公式のヘルプでは、グラフの入力欄に `piecewise(3,x>-2 and x<2)` のように書く、と説明されています。この形にならえば

```
piecewise((x-3)^2-2,x≥3)
```

で、$x \geq 3$ の枝だけが出ます。$\geq$ は**記号パレット**から入れられます（TI 公式のガイドでは、handheld で `ctrl` と `k` を押すとパレットが開く、と書かれています）。

**書き足さないかぎり、画面に出ているのは制限しない側の枝も含んだ絵です。** グラフを見て「対称になっている」と思っても、それだけでは足りません（[第8節](#restriction)）。
:::

### 4. 検算のしかた {#gdc-check}

- 合成の式を出したら、**$2$ つか $3$ つの値**で、$f(g(a))$ と自分の式の値が合うか見る
- 逆関数を出したら、**$f^{-1}$ に $f$ の出力を入れて、元の入力に戻るか**を見る（@eq-ahl27-undo）
- 逆関数のグラフを $y = x$ と一緒にかいて、**折り返しになっているか**を見る
- 制限つきの問題では、**$+$ と $-$ のどちらを選んだか**を、グラフの位置で確かめる

$2$ つ目がいちばん確実です。**戻らなければ、逆関数の式が違っています。**

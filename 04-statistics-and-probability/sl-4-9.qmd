...
ヒストグラムでは、**ある範囲に入るデータの割合が、その範囲の棒の面積**でした。棒を細くしていっても、この関係は変わりません。だから曲線になったあとも、
...
そして**全部のデータを合わせれば $100\%$** なので、**曲線の下の面積は全部で $1$** です。@fig-sl49-empirical の $99.7\%$ が「ほぼ全部」なのは、このためです。
...
**(a)** 左のはしから $56$ まで。**下限には $-1 \times 10^{99}$**（電卓では `-1E99`）を入れます。

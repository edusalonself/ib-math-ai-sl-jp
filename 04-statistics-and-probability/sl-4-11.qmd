...
**$p$-value** は、「$H_0$ が正しいとしたら、手元のようなデータが出る確率」です。**小さいほど $H_0$ が疑わしい**ということになります。
...
$$
p < \alpha \quad \Longrightarrow \quad \textbf{reject } H_0
$$

$$
p \geq \alpha \quad \Longrightarrow \quad \textbf{do not reject } H_0
$$
...
$$
\chi^2_{\text{calc}} > \chi^2_{\text{crit}} \quad \Longrightarrow \quad \textbf{reject } H_0
$$

$$
\chi^2_{\text{calc}} \leq \chi^2_{\text{crit}} \quad \Longrightarrow \quad \textbf{do not reject } H_0
$$

**$p$ のときと向きが逆**です。統計量は**大きい**ほど $H_0$ が怪しくなります。

critical value は問題文に与えられます。自分で表を引く必要はありません。
:::

::: {.callout-warning}
## 「accept $H_0$」と書いてはいけません
$p \geq \alpha$ のとき、正しい言い方は **do not reject $H_0$** です。

`accept H₀` や「$H_0$ が正しいと分かった」と書くと、**減点されます。**
...
- 表が出てきたら → **independence**
- 「割合はこうなるはず」と書いてあったら → **goodness of fit**
- 数値のリストが2つあって「平均」の話なら → **$t$-test**
...
**$p$-value は、「$H_0$ が正しいとして、$\chi^2$ が今回の値以上になる確率」**です。
...
::: {#exm-chi-indep}
[Paper 2]{.ib-tag} [A café records whether each customer orders tea or coffee, and their age group. The results are shown in the table below.]{.q-en}
...
**(c)** 電卓に表を行列として入れて、`χ² 2-way Test` を実行します（やり方は [Using your GDC](#using-your-gdc-ti-nspire-cx-ii) に書きました）。
...
::: {#exm-chi-critical}
[Paper 2]{.ib-tag} [A company tests four different advertisements. For each advertisement, it records whether people remembered the product afterwards.]{.q-en}
...
有意水準 $5\%$ で $\chi^2$ 独立性の検定を行います。臨界値は $7.815$ です。
...
::: {#exm-chi-gof}
[Paper 2]{.ib-tag} [A company claims that its bags of sweets contain red, green and yellow sweets in the ratio $2:3:5$. A sample of $200$ sweets contains $32$ red, $68$ green and $100$ yellow.]{.q-en}
...
::: {#exm-t-two-tailed}
[Paper 2]{.ib-tag} [Two classes take the same test. Their scores are shown below.]{.q-en}
...
**(b)** 2つのリストを電卓に入れて、`2-Sample t Test` を実行します。**`Pooled` は `Yes`** です（シラバスの指定）。
...
::: {#exm-t-one-tailed}
[Paper 2]{.ib-tag} [A gardener wants to test whether a new fertiliser makes plants taller. She grows $8$ plants with the fertiliser and $8$ plants without it, and measures their heights in cm after four weeks.]{.q-en}
...
`accept` は英語の型として間違いなので、そのまま減点されます。**書き出しごと覚えてください。**
...
- **$p$ は小さいと reject**（$p < \alpha$）
- **$\chi^2$ は大きいと reject**（$\chi^2 > \chi^2_{\text{crit}}$）
...
$p < \alpha$ で言えるのは「偶然では説明しにくい」ということだけです。**「主張が真である」ことは示していません。**
...
::: {.callout-tip}
## 期待度数も電卓が出してくれます
@exm-chi-critical の (a) のように「期待度数を求めよ」と言われたら、@eq-expected で手計算してもよいですし、**`ExpMatrix` を見ても構いません。**

ただし `Find the expected frequency` と単独で問われた場合は、**式を書いておくほうが安全**です。過程に点が付くことがあります。
...
[1]{.ex-no} [Paper 2]{.ib-tag} [A school records whether students in three year groups walk or cycle to school.]{.q-en}
...
**(a)** [Write down the number of degrees of freedom.]{.q-en} **(b)** [Find the $p$-value.]{.q-en} **(c)** [State the conclusion in context.]{.q-en}
...
**(a)** 自由度を書きなさい。 **(b)** $p$-value を求めなさい。 **(c)** 結論を文脈に沿って述べなさい。
...
[2]{.ex-no} [Paper 2]{.ib-tag} [A survey asks people in three cities which of three brands they prefer.]{.q-en}
...
**(a)** [Write down the number of degrees of freedom.]{.q-en} **(b)** [Find the $\chi^2$ statistic.]{.q-en} **(c)** [State, with a reason, the conclusion of the test.]{.q-en}
...
有意水準 $5\%$ で $\chi^2$ 独立性の検定を行います。臨界値は $9.488$ です。

**(a)** 自由度を書きなさい。 **(b)** $\chi^2$ 統計量を求めなさい。 **(c)** 検定の結論を、理由をつけて述べなさい。
...
[3]{.ex-no} [Paper 2]{.ib-tag} [A die is rolled $120$ times. The results are shown below.]{.q-en}
...
**(a)** [Write down the expected frequency for each score.]{.q-en} **(b)** [Write down the number of degrees of freedom.]{.q-en} **(c)** [Find the $p$-value and state the conclusion.]{.q-en}
...
**(a)** それぞれの目の期待度数を書きなさい。 **(b)** 自由度を書きなさい。 **(c)** $p$-value を求め、結論を述べなさい。
...
[4]{.ex-no} [Paper 2]{.ib-tag} [A shop expects its customers to choose small, medium and large drinks in the ratio $5:2.5:2.5$. On one day, $200$ customers choose $88$ small, $52$ medium and $60$ large.]{.q-en}

[A $\chi^2$ goodness of fit test is carried out at the $5\%$ significance level.]{.q-en}

**(a)** [Find the expected frequencies.]{.q-en} **(b)** [Find the $p$-value and state the conclusion in context.]{.q-en}
...
**(a)** 期待度数を求めなさい。 **(b)** $p$-value を求め、結論を文脈に沿って述べなさい。
...
[5]{.ex-no} [Paper 2]{.ib-tag} [A researcher wishes to test whether the colour of a car is independent of the country in which it was bought. She collects data from $4$ countries and $3$ colours.]{.q-en}
...
[6]{.ex-no} [Paper 2]{.ib-tag} [Two groups of students take the same test. Their scores are shown below.]{.q-en}
...
**(a)** [State the hypotheses.]{.q-en} **(b)** [Find the $p$-value.]{.q-en} **(c)** [State the conclusion in context.]{.q-en}
...
**(a)** 仮説を述べなさい。 **(b)** $p$-value を求めなさい。 **(c)** 結論を文脈に沿って述べなさい。
...
[7]{.ex-no} [Paper 2]{.ib-tag} [A coach claims that a new training programme increases the distance athletes can jump. Seven athletes follow the programme and seven do not. Their jump distances, in metres, are shown below.]{.q-en}
...
[8]{.ex-no} [Paper 2]{.ib-tag} [In a $\chi^2$ test for independence, the $p$-value is found to be $0.062$.]{.q-en}
...
[9]{.ex-no} [Paper 2]{.ib-tag} [For each of the following situations, state which test should be used: a $\chi^2$ test for independence, a $\chi^2$ goodness of fit test, or a $t$-test. Justify each answer.]{.q-en}
...
[10]{.ex-no} [Paper 2]{.ib-tag} [A researcher carries out a $t$-test to compare the mean lifetimes of two brands of battery. She finds $p = 0.003$ and concludes at the $5\%$ significance level that Brand A batteries last longer.]{.q-en}

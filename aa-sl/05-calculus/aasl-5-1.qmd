...
::: {.callout-warning}
## この項目は Paper 1 に出ます
Topic 5 は、AA では**ほとんどが電卓なしの項目**です。**このページの例題と演習は、計算が手でできる数にしてあります。**
:::
...
::: {#exm-aasl51-rate}
[The height of a plant, $y$ cm, is measured $t$ days after it is planted. The results are shown in the table.]{.q-en}

| $t$ (days) | $0$ | $2$ | $4$ | $6$ | $8$ |
|:--|--:|--:|--:|--:|--:|
| $y$ (cm) | $0$ | $3$ | $8$ | $15$ | $20$ |

**(a)** [Find the average rate of change of $y$ with respect to $t$ between $t = 2$ and $t = 6$, and state its units.]{.q-en}

**(b)** [Interpret the meaning of $\dfrac{dy}{dt}$ at $t = 4$ in this context.]{.q-en}
...
**(a)** [Find the gradient of the chord joining the points on the curve where $x = 1$ and $x = 1 + h$, for $h = 0.1$, $h = 0.01$, $h = -0.1$ and $h = -0.01$. Give each answer as a fraction.]{.q-en}
...
## 解答例（答案用紙に書くこと）
**(a)**
$$h = 0.1: \ -\frac{10}{11} \qquad h = 0.01: \ -\frac{100}{101}$$
$$h = -0.1: \ -\frac{10}{9} \qquad h = -0.01: \ -\frac{100}{99}$$
...
[2]{.ex-no} [Copy and complete the table below, and use it to estimate $\displaystyle\lim_{x \to 2} \dfrac{x^{2} - 4}{x - 2}$.]{.q-en}
...
[5]{.ex-no} [A function $f$ satisfies $f(2) = 5$ and $f(5) = 32$. Describe the difference in meaning between $\dfrac{f(5) - f(2)}{5 - 2}$ and $f'(5)$, and state which of the two can be found from the information given.]{.q-en}
...
割線の傾きと接線の傾きのちがいです（[第 1 節](#average)、[第 4 節](#tangent)）。
...
[7]{.ex-no} [The function $f$ is defined by $f(x) = 2x + 1$ for $x < 1$, and $f(x) = 5 - x$ for $x \geq 1$. Copy and complete the table below. Explain why $\displaystyle\lim_{x \to 1} f(x)$ does not exist, even though $f(1)$ does.]{.q-en}
...
左と右で使う式がちがうので、別々に見ます（[第 2 節](#limit)）。
...
[8]{.ex-no} [The function $f$ is given by $f(x) = \dfrac{1}{x}$. Find the gradient of the chord joining the points on the curve where $x = 2$ and $x = 2 + h$, for $h = 0.1$ and $h = -0.1$, giving each answer as a fraction. Hence write down an estimate of the gradient of the curve at $x = 2$.]{.q-en}
...
$f(2) = \dfrac{1}{2}$、$f(2.1) = \dfrac{10}{21}$、$f(1.9) = \dfrac{10}{19}$ です（[第 1 節](#average)）。

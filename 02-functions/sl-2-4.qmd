...
**asymptote**（漸近線）は、**グラフがどこまでも近づいていくのに、決して届かない線**です。
...
**式が計算できなくなる $x$** のところに立ちます。

@fig-sl24-asymptotes の右は $y = \dfrac{12}{x} + 3$ です。$x = 0$ では $\dfrac{12}{0}$ が計算できません。ですから
...
ここでは **asymptote に「届かない」のはなぜか**だけを説明します。
...
一方で、$x = 1000$ なら $\dfrac{12}{1000} = 0.012$ で $y = 3.012$。$x = 1000000$ なら $y = 3.000012$。**いくらでも近づけますが、届きません。** これが asymptote です。
...
**(a)** [計算できなくなる $x$](#vertical-asymptote) を探します。$\dfrac{12}{x}$ は $x = 0$ で計算できません。
...
3. **lower bound**（左の境目）をクリック
4. **upper bound**（右の境目）をクリック
...
**horizontal** … 表を出して、$x$ を大きくしたときの $y$ を見ます。
...
**vertical** … 式を見て、**分母が $0$ になる $x$** を探します。$\dfrac{12}{x}$ なら $x = 0$、$\dfrac{5}{x - 2}$ なら $x = 2$ です。
...
::: {.callout-tip collapse="true"}
## 解説
**(a)** vertical は「計算できなくなる $x$」なので $x = 0$。horizontal は $x$ を大きくすると $\dfrac{8}{x}$ が $0$ に近づくので、残る $-2$ です。
